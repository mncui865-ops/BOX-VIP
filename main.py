# -*- coding: utf-8 -*-
"""
FBI SUDANESE — v6.5 (Railway Ready)
- 4 أزرار تواصل: Telegram×2 + Facebook×2
- ID القروب بارز مع زر نسخ
- طلبات انضمام للقروبات + قبول/رفض
- نقل ملكية القروب تلقائياً
- صفحة حظر مخصصة
- بلاغات تعرض صاحب الشوكة
- ✅ v6.3: الردود + التفاعل + صورة المجموعة
- ✅ v6.4: زر نسخ في كل مكان + مراجعة الدردشات + تعليق الحساب
- ✅ v6.5 الجديد:
  * إصلاح جميع أخطاء f-string (\')
  * حالة خاصة (Friends only) 👥 + حالة عامة 🌍
  * اختيار الخصوصية عند النشر
  * الفيد يعرض الحالات حسب الخصوصية
"""
...
class Status(db.Model):
    ...
    privacy = db.Column(db.String(10), default="public")  # ✅ public / friends
...
<div class="card center" style="border:2px solid #fcd34d;background:#fffbeb">
  ...
  <div style="background:#fff;border:1px solid #fcd34d;border-radius:12px;
    ...
    <p style="margin-top:10px"><b>ما الذي يحدث الآن؟</b></p>
    <ul style="margin:6px 18px;font-size:13px">
      <li>فريق الدعم يراجع المحادثة المبلَّغ عنها حالياً</li>
      <li>لا يمكنك استخدام الموقع مؤقتاً حتى يتم البتّ في المراجعة</li>
      <li>إذا تم رفض البلاغ → يرجع حسابك كاملاً</li>
      <li>إذا تم تأييد البلاغ → سيُحظر حسابك نهائياً</li>
    </ul>
...
.inline-uid{display:inline-flex;align-items:center;gap:4px}
</style>
"""
    ...
    for u in pagination.items:
        ...
        rows += f"""<div class="list-item" style="cursor:default">
          <a href="{url_for('view_profile', username=u.username)}" style="display:flex;align-items:center;gap:10px;flex:1;text-decoration:none;color:inherit;min-width:0">
            ...
            <div style="flex:1;min-width:0"><div class="li-name">{u.short_name}</div>
...
def post_status():
    if request.method == "POST":
        ...
        flash(f"تم نشر الحالة ({label}). تنتهي تلقائيًا بعد 24 ساعة.", "success")
        return redirect(url_for("feed"))
    return render_page(FLASH_BLOCK + topbar_html(badge_html()) + """
...
def feed():
    ...
    # ✅ v6.5 — نجيب كل الحالات خلال 24 ساعة
    all_statuses = Status.query.filter(Status.created_at >= cutoff).order_by(Status.created_at.desc()).all()
    # فلترة: الحالات العامة + حالاتي + حالات الأصدقاء الخاصة + استثناء المحظورين
    visible = []
...
def render_bubble_chat(m, current_user, peer):
    ...
    peer_name_js = "أنت" if m.sender_id == current_user.id else peer.short_name
    peer_name_js = peer_name_js.replace("'", "\\'")
    return f"""<div class="bubble-wrap">
      <div class="bubble {cls} clickable" id="msg-{m.id}" onclick="toggleMsgBar({m.id})">
...
def chat_report(username):
    ...
    if existing:
        if existing.status == "pending":
            return jsonify({"ok": False, "error": "لديك بلاغ قيد المراجعة بالفعل على هذه المحادثة"}), 400
    ...
    db.session.commit()
    return jsonify({"ok": True, "message": "تم إرسال الإبلاغ — الحساب الآن تحت مراجعة فريق الدعم"})
...
function reportChat(){{
  if(!confirm('🚨 هل أنت متأكد من إبلاغ فريق الدعم عن هذه المحادثة؟\\nسيتم مراجعة حساب الطرف الآخر بالكامل.')) return;
  ...
  fetch('{report_url}',{{
    ...
    if(d.ok){{
      alert('✅ تم إرسال الإبلاغ.\\nسيقوم فريق الدعم بمراجعة الحساب والمحادثة.');
    }}else{{
...
def group_avatar_full(gid):
    ...
    note = ""
    if not is_admin:
        note = '<div style="font-size:11px;color:#9ca3af;margin-top:6px">🔒 التعديل متاح فقط للمالك/المشرف</div>'
    return render_page(f"""
    <div class="card center">
...
def group_view(gid):
    ...
    if g.avatar:
        if is_admin:
            avatar_note = '<div style="font-size:11px;color:#9ca3af;margin-top:6px">🔍 اضغط على الصورة لعرضها كاملة — يمكنك تغييرها من زر تعديل المجموعة</div>'
        else:
            avatar_note = '<div style="font-size:11px;color:#9ca3af;margin-top:6px">🔍 اضغط على الصورة لعرضها كاملة</div>'
    return render_page(FLASH_BLOCK + topbar_html(badge_html()) + f"""
    <div class="card center">
      {avatar_html}
      {avatar_note}
...
def render_group_bubble(m, current_user):
    ...
    if m.reaction:
        reaction_html = f'<span class="reaction-badge">{html_escape_text(m.reaction)}</span>'
    body_esc_js = (m.body or "").replace("\\", "\\\\").replace("'", "\\'").replace("\n", " ")[:60]
    sender_name_js = ("أنت" if m.sender_id == current_user.id else sender_name).replace("'", "\\'")
    return f"""<div class="bubble-wrap">
      <div class="bubble {cls} clickable" id="gmsg-{m.id}" onclick="toggleMsgBarG({m.id})">
...
def support_chat_reports():
    ...
    for r in reports:
        ...
        status_map = {
            ...
            "dismissed": '<span class="blocked-badge" style="background:#dcfce7;color:#166534;border-color:#86efac">✅ مرفوض — أُعيد الحساب</span>',
        }
        ...
        if r.status == "pending":
            ...
            <a class="btn btn-sm btn-danger" href="{upheld_url}"
               onclick="return confirm('🚫 تأييد البلاغ → حظر الحساب نهائياً وحذف بياناته؟')">🚫 تأييد + حظر</a>
            <a class="btn btn-sm btn-primary" href="{dismissed_url}"
               onclick="return confirm('✅ رفض البلاغ → إرجاع الحساب لحالته الطبيعية؟')">✅ رفض + إرجاع</a>"""
        elif r.status == "upheld":
            undo_url = url_for('support_review_chat_report', rid=r.id, decision='dismissed')
            actions = f'<a class="btn btn-sm" href="{undo_url}">↩ إعادة النظر (رفض + إرجاع)</a>'
        else:
            undo_url = url_for('support_review_chat_report', rid=r.id, decision='upheld')
            actions = f'<a class="btn btn-sm" href="{undo_url}">↩ إعادة النظر (تأييد + حظر)</a>'
        reporter_html = "مجهول"
        if rep:
            ...
            {copy_btn_html(rep.username, "يوزر", small=True)}
            {copy_btn_html(rep.public_id, "ID", small=True)}"""
        rows += f"""<div class="list-item" style="cursor:default;flex-wrap:wrap;border-right:4px solid #f59e0b">
          ...
          <div style="flex:1;min-width:240px">
            <div class="li-name">🎯 @{tgt.username} <span style="color:#6b7280;font-weight:400">({tgt.short_name})</span> {target_state} {status_badge}</div>
            <div class="li-sub">ID: {tgt.public_id} {copy_btn_html(tgt.public_id, "ID", small=True)} · بلاغات: {tgt.reports_count or 0}</div>
            <div class="li-sub" style="margin-top:4px">🪪 المُبلِّغ: {reporter_html}</div>
            <div class="li-sub" style="margin-top:4px">📅 {fmt_sd(r.created_at)} {('· تم البتّ: ' + fmt_sd(r.resolved_at)) if r.resolved_at else ''}</div>
          ...
          <div class="li-actions" style="flex-direction:column;gap:4px">
            <a class="btn btn-sm" href="{url_for('support_user_detail', uid=tgt.id)}">👁 فحص الحساب</a>
            <a class="btn btn-sm" href="{url_for('view_profile', username=tgt.username)}">👤 البروفايل</a>
            {actions}
          </div></div>"""
    if not rows:
    ...
    <div style="font-size:12px;color:#6b7280;margin-bottom:12px">
      ⚠️ عند التأييد: الحساب يُحظر نهائياً وتُحذف كل بياناته.<br>
      ✅ عند الرفض: الحساب يرجع لحالته الطبيعية ويُرفع التعليق.
    </div>
    {rows}
...
def support_review_chat_report(rid, decision):
    ...
    if decision == "upheld":
        ...
        flash(f"🚫 تم تأييد البلاغ وحظر @{tgt.original_username or tgt.username} نهائياً.", "success")
    else:
        r.status = "dismissed"
        r.resolved_at = now_utc_naive()
        ...
        db.session.commit()
        flash(f"✅ تم رفض البلاغ وإرجاع الحساب @{tgt.username} لحالته الطبيعية.", "success")
    return redirect(url_for("support_chat_reports"))
...
def support_user_detail(uid):
    ...
    for cr in chat_reports_against:
        ...
        status_map = {
            "pending": '<span class="blocked-badge" style="background:#fef3c7;color:#92400e;border-color:#fcd34d">⏳ قيد المراجعة</span>',
            "upheld": '<span class="blocked-badge" style="background:#fee2e2;color:#991b1b">🚫 مؤيَّد</span>',
    ...
    if u.under_review:
        release_review_btn = (f'<a class="btn btn-primary" href="{url_for("support_release_review", uid=u.id)}" '
                              f'onclick="return confirm(\'رفع تعليق المراجعة وإرجاع الحساب؟\')">✅ رفع المراجعة وإرجاع الحساب</a>')
    ...
    <div class="card center">
      ...
      {f'<div class="privacy-note" style="margin-top:10px">⏳ السبب: {html_escape_text(u.review_reason)}<br>منذ: {fmt_sd(u.review_started_at)}</div>' if u.under_review else ''}
      <div class="acct-box" style="margin-top:12px">
    ...
    <div class="card"><h2>💬 بلاغات الدردشة ضده ({len(chat_reports_against)})</h2>
    {chat_reports_html}</div>
    <div class="card"><h2>🚨 البلاغات العادية ضده ({len(reports_against)})</h2>
