# -*- coding: utf-8 -*-
"""
FBI SUDANESE — v6.6 (Railway Ready + Fixed Multi-Ban)
- ✅ إصلاح حظر عدة مستخدمين بدون صفحة بيضاء
- ✅ process_auto_ban محصّن + يحذف كل الجداول المرتبطة
- ✅ معالجة أخطاء 500 واضحة
"""
...
# ═══════════════════════════════════════════════════════════════
# ✅ process_auto_ban — النسخة المُحصّنة الكاملة
# ═══════════════════════════════════════════════════════════════
def process_auto_ban(user):
    """حظر مستخدم مع حذف كل بياناته بشكل آمن. لا يسبب صفحة بيضاء."""
    ...
    try:
        ...
        # 12) بلاغات الدردشة (كمُبلِّغ أو كهدف)
        ...
        # 14) ردود رسائل المجموعات (reply_to_id)
        ...
        # 21) مسح البيانات الشخصية + تغيير اليوزر
        user.bio = ""
        user.nickname = ""
        user.recovery_hash = ""
        ...
        traceback.print_exc()
        # محاولة أخيرة: علم الحظر فقط بدون حذف
        try:
...
<div class="card center" style="border:2px solid #fecaca;background:#fef2f2">
  ...
  <div style="background:#fff;border:1px solid #fecaca;border-radius:12px;
    ...
    <p><b>السبب:</b> تم حظر حسابك بسبب مخالفة شروط الاستخدام أو تجاوز عدد البلاغات.</p>
...
def internal_error(e):
    ...
    <div class="card center">
      ...
      <p style="color:#6b7280;margin:14px 0;font-size:14px">حدث خطأ غير متوقع. تم تسجيله للمراجعة.</p>
...
# ═══════════════════════════════════════════════════════════════
# ✅ BAN/UNBAN — النسخة المُصلحة (تعمل مع أي عدد من المستخدمين)
# ═══════════════════════════════════════════════════════════════
...
def support_ban_user(uid):
    ...
    if ok:
        flash(f"🚫 تم حظر @{saved_username} بنجاح وحذف كل بياناته.", "success")
    else:
        flash(f"⚠️ فشل حظر @{saved_username}. راجع سجلات السيرفر.", "error")
    return redirect(request.referrer or url_for("support_reports"))
