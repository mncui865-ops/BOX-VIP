# -*- coding: utf-8 -*-
"""
FBI SUDANESE — v6.2 (Railway Ready)
- 4 أزرار تواصل بنفس التنسيق: Telegram×2 + Facebook×2 (القديم + الجديد)
- ID القروب بارز مع زر نسخ
- طلبات انضمام للقروبات + قبول/رفض
- نقل ملكية القروب تلقائياً عند مغادرة المالك
- صفحة حظر مخصصة + زر خروج
...
<div class="card center" style="border:2px solid #fecaca;background:#fef2f2">
  ...
  <div class="actions" style="margin-top:16px">
    ...
    <a class="btn" href="{{ url_for('support_login') }}">🛡 التواصل مع الدعم</a>
  </div>
</div>
"""
...
def group_view(gid):
    ...
    return render_page(FLASH_BLOCK + topbar_html(badge_html()) + f"""
    <div class="card center">
      <img class="avatar" src="{group_avatar_url(g)}" style="border-radius:20px;width:100px;height:100px;cursor:default">
        ...
        if user_reports:
            ...
            for r in user_reports:
                ...
                if target.is_banned:
                    ...
                    target_ban_btn = f'<a class="btn btn-sm btn-danger" href="{url_for("support_ban_user", uid=target.id)}" onclick="return confirm(\'حظر هذا المستخدم (الهدف) نهائياً؟\')">🚫 حظر الهدف</a>'
                reporter_box = ""
                if reporter:
                    ...
                    reporter_box = f"""<div style="background:#fef2f2;border:1px dashed #fecaca;
                        ...
                        <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
                          ...
                          <a href="{url_for('view_profile', username=reporter.username)}" style="flex:1;text-decoration:none;color:inherit">
                ...
                users_html += f"""<div class="list-item" style="cursor:default;flex-wrap:wrap;border-right:4px solid #2563eb">
                  ...
                  <div style="flex:1;min-width:200px">
                    ...
                    <div style="font-size:13px;margin-top:6px;color:#374151;background:#fef3c7;border-radius:6px;padding:6px 10px">
                      💬 {r.reason or 'بدون سبب مذكور'}</div>
                    {reporter_box}
                  </div>
...
def support_users():
    ...
    for u in users:
        ...
        rows += f"""<div class="list-item" style="cursor:default">
          ...
          <div class="li-actions">{action}</div></div>"""
    if not rows:
        rows = '<p class="empty">لا نتائج.</p>'
    return render_page(FLASH_BLOCK + f"""
