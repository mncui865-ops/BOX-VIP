...
"""
╔══════════════════════════════════════════════════════════╗
║              𝙵𝙱𝙸 𝚂𝚄𝙳𝙰𝙽𝙴𝚂𝙴  —  v3.1 (Railway Ready)      ║
╠══════════════════════════════════════════════════════════╣
║  ✅ حالات + دردشة + رسائل مجهولة + أصدقاء                ║
║  ✅ دعم Railway Volumes (استمرارية الملفات)              ║
║  ✅ دعم PostgreSQL (استمرارية قاعدة البيانات)            ║
║  ✅ متغيرات البيئة + PORT التلقائي                        ║
║  ❌ المنشورات (تمت إزالتها بالكامل)                       ║
╚══════════════════════════════════════════════════════════╝
"""
...
def render_page(body, title=None, **ctx):
    ...
    return render_template_string(
        ...
        "function copyText(t,b){navigator.clipboard.writeText(t).then(function(){"
        ...
        "</div>"
        "<div id='copy-toast' style='display:none;position:fixed;bottom:20px;left:50%;"
        "transform:translateX(-50%);background:#111827;color:#fff;padding:10px 20px;"
        "border-radius:10px;font-size:14px;z-index:3000'>✓ تم النسخ</div>"
        "</body></html>",
...
def view_profile(username):
    ...
    if is_own or is_friend:
        ...
        <div class="acct-box" style="margin-top:12px">
          ...
          <div class="acct-row"><span class="k">USERNAME</span>
            <span class="v">@{user.username}</span></div>
          <div class="acct-row"><span class="k">ID</span>
            <span class="v">{user.public_id}</span></div>
    ...
    if is_own:
        ...
        <a class="btn btn-disabled">💬 الدعم — غير متوفر حاليًا</a>
...
def edit_profile():
    ...
    <input name="nickname" value="{{ user.nickname or '' }}" maxlength="64" required>
    <label>نبذة عنك</label>
    <textarea name="bio" maxlength="200">{{ user.bio or '' }}</textarea>
    <label>صورة البروفايل</label>
    <input type="file" name="avatar" accept="image/*">
    <button type="submit">حفظ</button></form>
    <a class="link-center" href="{{ url_for('profile_me') }}">رجوع</a>
    </div>""", title="تعديل", user=current_user)
...
def friends_list():
    ...
    <div class="card">
      ...
      <div style="display:flex;gap:8px;margin-bottom:12px">
        ...
        <a class="btn btn-sm" href="{url_for('search')}">إضافة أصدقاء</a>
      </div>
      {rows}
    </div>""", title="الأصدقاء")
...
def search():
    ...
    for u in users_results:
        ...
        <div class="li-sub">@{u.username}</div></div>
        <div class="li-actions">
          <a class="btn btn-sm btn-primary" href="{url_for('chat_with', username=u.username)}">دردشة</a>
        </div></a>"""
...
def feed():
    ...
    for s in statuses:
        if s.user_id not in seen_users:
            ...
            statuses_unique.append(s)

    status_strip = ""
    for s in statuses_unique:
        av = avatar_url(s.user)
        tag = '<span class="vid-tag">▶</span>' if s.media_type == "video" else ""
        status_strip += f"""<a class="status-item" href="javascript:void(0)"
            onclick="openStatus({s.id})">
          <span class="ring"><img src="{av}">{tag}</span>
          <div class="sname">{s.user.short_name}</div>
        </a>"""
