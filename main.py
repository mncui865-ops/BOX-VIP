# -*- coding: utf-8 -*-
"""
FBI SUDANESE — v12.0 (Telegram Verified Edition — Full Fixed)
- ✅ إصلاح جميع الأخطاء الحرجة
- ✅ علامة توثيق مثل تلغرام (أي لون + أي أيقونة)
- ✅ غلاف بروفايل + خلفية دردشة شخصية
- ✅ كود استعادة + كتابة كلمة السر الجديدة
- ✅ نقطة خضراء/سوداء + آخر ظهور
- ✅ رسائل مختفية + عرض مرة واحدة + ردود سريعة
- ✅ حالة مزاجية + ألقاب مخصصة + ثيمات
- ✅ إيموجيات مفضلة
- ✅ حفظ جميع الميزات القديمة + تحسينات
"""
...
class User(UserMixin, db.Model):
    ...
    mood_emoji = db.Column(db.String(10), default="")
    ...
    theme_primary = db.Column(db.String(20), default="#111827")
    theme_accent = db.Column(db.String(20), default="#2563eb")
    chat_wallpaper = db.Column(db.String(300), default="")
    favorite_emojis = db.Column(db.String(200), default="❤️,👍,😂,😮,😢,🙏")
    default_disappear_seconds = db.Column(db.Integer, default=0)  # 0 = off
...
def cleanup_expired_messages():
    """✅ حذف الرسائل المختفية التي انتهى وقتها"""
    now = now_utc_naive()
    expired = ChatMessage.query.filter(
...
def cleanup_view_once_messages():
    """✅ حذف الرسائل التي شوهدت مرة واحدة"""
        ...
        try:
            ...
            for tbl, cols in [
                ("chat_message", [("reply_to_id", "INTEGER"), ("reaction", "VARCHAR(16) DEFAULT ''"),
                                   ...
                                   ("viewed_once", "BOOLEAN DEFAULT FALSE"),
                                   ("edited_at", "TIMESTAMP")]),
                ...
                for col, typ in cols:
                    if col not in existing:
                        ...
                        db.session.commit()
            for tbl_sql in [
                ("typing_indicator", """CREATE TABLE IF NOT EXISTS typing_indicator (
                    id INTEGER PRIMARY KEY, user_id INTEGER NOT NULL,
                    peer_id INTEGER, group_id INTEGER, updated_at TIMESTAMP)"""),
                ("quick_reply", """CREATE TABLE IF NOT EXISTS quick_reply (
                    id INTEGER PRIMARY KEY, user_id INTEGER NOT NULL,
                    text VARCHAR(200) NOT NULL, created_at TIMESTAMP)"""),
            ]:
                try:
                    insp.get_columns(tbl_sql[0])
                except Exception:
                    db.session.execute(text(tbl_sql[1]))
                    db.session.commit()
        except Exception as e:
...
def cover_url(user):
    if not user or not user.profile_cover:
        return ""
...
def verified_html(user, large=False):
    if user and getattr(user, "is_verified", False):
        ...
        font_size = "14px" if large else "11px"
        cls = "verified-badge lg" if large else "verified-badge"
        return (f'<span class="{cls}" title="حساب موثّق" '
                f'style="background:{color};width:{size};height:{size};'
                f'font-size:{font_size}">{icon}</span>')
    return ""
...
def online_status_html(user, show_text=True):
    ...
    if user.is_online:
        ...
        text = '<span class="status-text online-text">متصل</span>' if show_text else ''
    else:
...
background-size:cover;background-position:center;background-attachment:local;position:relative;}
.chat-box::before{content:"";position:absolute;inset:0;background:rgba(247,247,248,.82);
border-radius:var(--radius);pointer-events:none;z-index:0;}
body.dark .chat-box::before{background:rgba(15,23,42,.85);}
...
.verified-badge{display:inline-flex;align-items:center;justify-content:center;
  ...
  font-size:11px;margin-right:4px;flex-shrink:0;font-weight:900;line-height:1;
...
.verified-badge::after{content:"";position:absolute;inset:0;border-radius:50%;
  ...
  pointer-events:none;}
.verified-badge.lg{width:22px;height:22px;font-size:14px;}
...
.dark-toggle .circle{width:24px;height:24px;border-radius:50%;
  ...
  font-size:13px;line-height:1;position:absolute;top:50%;transform:translateY(-50%);}
...
body.dark .dark-toggle .circle.sun{opacity:0;transform:translateY(-50%) scale(.5);}
body.dark .dark-toggle .circle.moon{opacity:1;transform:translateY(-50%) scale(1);}
body:not(.dark) .dark-toggle .circle.moon{opacity:0;transform:translateY(-50%) scale(.5);}
body:not(.dark) .dark-toggle .circle.sun{opacity:1;transform:translateY(-50%) scale(1);}
...
.status-dot{position:absolute;bottom:4px;right:4px;width:20px;height:20px;border-radius:50%;
  border:3px solid var(--surface);box-shadow:0 1px 4px rgba(0,0,0,.2);z-index:3;
  transition:.2s;}
.status-dot.online{background:#22c55e;animation:pulseOnline 2s infinite;}
.status-dot.offline{background:#1f2937;}
body.dark .status-dot.offline{background:#000;border-color:#1e293b;}
@keyframes pulseOnline{
  0%,100%{box-shadow:0 0 0 3px var(--surface),0 0 0 0 rgba(34,197,94,.7);}
  50%{box-shadow:0 0 0 3px var(--surface),0 0 0 8px rgba(34,197,94,0);}
}
.status-text{font-size:13px;font-weight:700;margin-top:6px;display:inline-block;}
.status-text.online-text{color:#22c55e;}
.status-text.offline-text{color:#9ca3af;font-weight:500;font-size:12px;}
body.dark .status-text.offline-text{color:#64748b;}
...
.profile-cover-wrap .cover-edit-btn:hover{background:rgba(0,0,0,.85);transform:scale(1.03);}
.profile-avatar-overlap{margin-top:-50px;position:relative;z-index:2;}

.mood-badge{display:inline-flex;align-items:center;gap:5px;background:rgba(255,255,255,.9);
  border:1px solid var(--border);border-radius:999px;padding:4px 12px;font-size:13px;
  font-weight:600;margin-top:8px;box-shadow:0 2px 6px rgba(0,0,0,.06);}
body.dark .mood-badge{background:#1e293b;}
...
/* ✅ رسالة عرض مرة واحدة */
.viewonce-bubble{cursor:pointer;position:relative;}
.viewonce-overlay{position:absolute;inset:0;background:rgba(37,99,235,.15);
...
.disappear-selector .dopt:hover{background:#fef3c7;}
</style>
"""
...
def render_page(body, title=None, **ctx):
    ...
    return render_template_string(
        ...
        "<span class='circle sun'>☀</span>"
        "<span class='circle moon'>🌙</span>"
        "</button>"
        "<div class='container'>"
        ...
        "var all=tmp.querySelectorAll('.bubble-wrap');"
        "var lastId=all.length?all[all.length-1].querySelector('.bubble').id:'';"
        "if(_lastMsgId&&lastId&&lastId!==_lastMsgId){"
        ...
        "g.gain.value=0.05;o.start();o.stop(ctx.currentTime+0.08);}catch(e){}"
        "}"
        "_lastMsgId=lastId;"
...
def recover():
    ...
    if request.method == "POST":
        ...
        if len(new_password) < 8:
            flash("كلمة المرور الجديدة: 8 أحرف على الأقل.", "error")
            return redirect(url_for("recover"))
        ...
        flash(f"تم الدخول وتغيير كلمة السر. أهلاً @{user.username}!", "success")
        resp = make_response(redirect(url_for("welcome")))
        resp.set_cookie("device_token", token, max_age=60*60*24*365,
                        httponly=True, samesite="Lax",
                        secure=app.config["SESSION_COOKIE_SECURE"])
    ...
    <p style="color:#6b7280;font-size:13px;text-align:center;margin-bottom:16px">
      أدخل الكود وكلمة السر الجديدة.</p>
    <form method="POST" onsubmit="return validateRecover()">
      ...
      <button type="submit">🔓 استعادة وتعيين كلمة السر</button>
    </form>
    ...
    function validateRecover(){
      ...
      var p1=document.getElementById('npw').value, p2=document.getElementById('npw2').value;
      if(p1.length<8){alert('كلمة المرور: 8 أحرف على الأقل');return false;}
...
def before_each_request():
    ...
    allowed_special = {
        ...
        "support_verify_user", "support_unverify_user",
...
# ═══════════════════════════════════════════════════════════════
# PROFILE + FRIENDS + BLOCK + ARCHIVE + COVER
# ═══════════════════════════════════════════════════════════════
    ...
    if request.method == "POST":
        ...
        if file and file.filename:
            if not allowed_image(file.filename) or not check_image_magic(file):
                ...
                return redirect(url_for("edit_cover"))
    ...
    if current_user.profile_cover:
        ...
        <form method="POST" style="margin-top:10px">
          ...
          <button type="submit" class="btn btn-danger" style="width:100%"
                  onclick="return confirm('حذف الغلاف؟')">🗑 حذف الغلاف الحالي</button>
        </form>"""
...
def view_profile(username):
    ...
    # ✅✅ إصلاح الغلاف (كان فيه if cu: فارغة)
    cu = cover_url(user)
    if cu:
        if is_own:
            cover_html = f"""<div class="profile-cover-wrap">
              <img src="{cu}" onclick="openAvatar('{cu}')">
              <button class="cover-edit-btn" onclick="window.location.href='{url_for('edit_cover')}'">🖼 تغيير الغلاف</button>
            </div>"""
        else:
        ...
        if is_own:
            cover_html = (f'<div class="profile-cover-wrap no-cover" style="background:linear-gradient(135deg,#667eea,#764ba2)">'
                          f'<button class="cover-edit-btn" onclick="window.location.href=\'{url_for("edit_cover")}\'">➕ إضافة غلاف</button>'
                          f'</div>')
        else:
    ...
    mood_html = ""
    if user.mood_text or user.mood_emoji:
        mood_html = f'<div class="mood-badge"><span class="mood-emoji">{html_escape_text(user.mood_emoji)}</span>{html_escape_text(user.mood_text)}</div>'

    title_html = ""
    if user.custom_title:
        tc = user.title_color or "#6b7280"
        title_html = f'<div class="custom-title" style="background:{tc}">{html_escape_text(user.custom_title)}</div>'
    ...
    if is_own or is_friend:
        ...
        {title_html}
        {mood_html}
        <div class="status-line">{dot_html}{status_text_html}</div>
        <div class="uid" style="margin-top:10px">ID: {user.public_id}</div>
        ...
        profile_html = f"""
        <div class="name">{verified_big} @{user.username}</div>
        {title_html}
        {mood_html}
        <div class="status-line">{dot_html}{status_text_html}</div>
        <div class="uid" style="margin-top:10px">ID: {user.public_id} {copy_btn_html(user.public_id, "نسخ", small=True)}</div>
    ...
    if is_own:
        ...
        <a class="btn" href="{url_for('edit_cover')}">🖼 تعديل الغلاف</a>
        <a class="btn" href="{url_for('edit_chat_wallpaper')}">🖼 خلفية الدردشة</a>
        <a class="btn" href="{url_for('edit_extras')}">✨ المميزات الإضافية</a>
        <a class="btn" href="{url_for('quick_replies')}">⚡ الردود السريعة</a>
...
def edit_extras():
    if request.method == "POST":
        current_user.mood_emoji = request.form.get("mood_emoji", "").strip()[:4]
        ...
        if tc.startswith("#") and len(tc) in (4, 7):
            current_user.title_color = tc
        tp = request.form.get("theme_primary", "#111827").strip()
        if tp.startswith("#") and len(tp) in (4, 7):
            current_user.theme_primary = tp
        ta = request.form.get("theme_accent", "#2563eb").strip()
        if ta.startswith("#") and len(ta) in (4, 7):
            current_user.theme_accent = ta
        ...
        try:
            ...
            pass
        db.session.commit()
        flash("تم حفظ المميزات الإضافية.", "success")
        return redirect(url_for("profile_me"))
    dsec = current_user.default_disappear_seconds or 0
    def sel(v): return "selected" if dsec == v else ""
    return render_page(FLASH_BLOCK + topbar_html(badge_html()) + f"""
    <div class="card"><h2>✨ المميزات الإضافية</h2>
    <form method="POST">
      <label>😊 حالة مزاجية — إيموجي</label>
      <input name="mood_emoji" maxlength="4" value="{current_user.mood_emoji or ''}" placeholder="مثال: 😎">

      <label>💬 حالة مزاجية — نص</label>
      ...
      <div style="display:flex;gap:8px;align-items:center">
        ...
        <input type="text" value="{current_user.title_color or '#6b7280'}" disabled style="flex:1;direction:ltr">
      </div>

      <label>🎨 اللون الأساسي للثيم</label>
      <div style="display:flex;gap:8px;align-items:center">
        <input type="color" name="theme_primary" value="{current_user.theme_primary or '#111827'}"
               style="width:60px;height:44px;padding:2px;cursor:pointer">
        <input type="text" value="{current_user.theme_primary or '#111827'}" disabled style="flex:1;direction:ltr">
      </div>

      <label>🎨 لون التمييز</label>
      <div style="display:flex;gap:8px;align-items:center">
        <input type="color" name="theme_accent" value="{current_user.theme_accent or '#2563eb'}"
               style="width:60px;height:44px;padding:2px;cursor:pointer">
        <input type="text" value="{current_user.theme_accent or '#2563eb'}" disabled style="flex:1;direction:ltr">
      </div>

      <label>❤️ الإيموجيات المفضلة (مفصولة بفاصلة)</label>
      <input name="favorite_emojis" maxlength="200" value="{current_user.favorite_emojis or ''}"
             placeholder="❤️,👍,😂,😮,😢,🙏" style="direction:ltr">
      ...
      <select name="default_disappear_seconds" style="width:100%;padding:12px;border-radius:10px;border:1px solid var(--border);background:var(--surface);color:var(--text)">
        <option value="0" {sel(0)}>🚫 مغلق (لا تختفي)</option>
      ...
      <button type="submit" style="margin-top:16px">💾 حفظ المميزات</button>
    </form>
    <a class="link-center" href="{url_for('profile_me')}">رجوع</a></div>""", title="المميزات الإضافية")
...
def quick_replies():
    if request.method == "POST":
        ...
        if text and len(text) <= 200:
            existing = QuickReply.query.filter_by(user_id=current_user.id).count()
            if existing >= 20:
                flash("الحد الأقصى 20 رد سريع.", "error")
            else:
                db.session.add(QuickReply(user_id=current_user.id, text=text))
                db.session.commit()
                flash("تم إضافة الرد السريع.", "success")
        return redirect(url_for("quick_replies"))
    replies = QuickReply.query.filter_by(user_id=current_user.id).order_by(
        QuickReply.created_at.desc()).all()
    rows = ""
    for r in replies:
        rows += f"""<div class="list-item" style="cursor:default">
          <div style="flex:1">💬 {html_escape_text(r.text)}</div>
          <a class="btn btn-sm btn-danger" href="{url_for('quick_reply_delete', rid=r.id)}"
             onclick="return confirm('حذف؟')">🗑</a>
        </div>"""
    if not rows:
        rows = '<p class="empty">لا ردود سريعة.</p>'
    return render_page(FLASH_BLOCK + topbar_html(badge_html()) + f"""
    <div class="card"><h2>⚡ الردود السريعة ({len(replies)}/20)</h2>
    <form method="POST" style="margin-bottom:14px">
      <label>إضافة رد سريع جديد</label>
...
def devices_list():
    ...
    for d in devices:
        ...
        last = fmt_sd(d.last_seen)
        rows += f"""<div class="device-row">
        <div style="flex:1"><div><b>جهاز</b> — {created} (SD)</div>
        <div class="ua">{ua}</div>
        <div style="font-size:11px;color:#6b7280">آخر نشاط: {last}</div></div>
...
def starred_messages():
    ...
    for m in msgs:
        ...
        body_txt = m.body or ("📷 صورة" if m.media_type == "image" else
                              "🎥 فيديو" if m.media_type == "video" else
                              "🎵 أغنية" if m.media_type == "audio" else "مرفق")
    ...
    if not rows:
        rows = '<p class="empty">لا رسائل مثبتة.</p>'
    return render_page(FLASH_BLOCK + topbar_html(badge_html()) + f"""
...
def delete_account():
    if request.method == "POST":
        ...
        try:
            ...
            if current_user.profile_cover:
                try:
                    p = os.path.join(app.config["COVER_FOLDER"], current_user.profile_cover)
                    if os.path.exists(p): os.remove(p)
                except OSError: pass
            if current_user.chat_wallpaper:
                try:
                    p = os.path.join(app.config["COVER_FOLDER"], current_user.chat_wallpaper)
                    if os.path.exists(p): os.remove(p)
                except OSError: pass
...
def chats():
    ...
    for pinned, _, f, last_msg, unread, muted in items:
        ...
        rows += f"""<div class="list-item">
          ...
          <a href="{url_for('chat_with', username=f.username)}" style="flex:1;text-decoration:none;color:inherit;min-width:0">
            <div class="li-name">{pin_icon}{mute_icon}{verified_html(f)}{f.short_name} {unread_tag}</div>
    ...
    <div style="display:flex;gap:8px;margin-bottom:12px;flex-wrap:wrap">
      ...
      <a class="btn btn-sm" href="{url_for('quick_replies')}">⚡ ردود سريعة</a>
      {archive_link}
...
def render_bubble_chat(m, current_user, peer):
    ...
    header_html = ""
    if is_mine:
        my_avatar = avatar_url(current_user)
        my_verified = verified_html(current_user)
        header_html = f"""<a class="sender-head" href="{url_for('profile_me')}"
           onclick="event.stopPropagation()">
           <img src="{my_avatar}" class="sender-avatar">
           <span class="sender-name">{my_verified}{html_escape_text(current_user.short_name)}
           <span style="color:#16a34a;font-size:10px">(أنت)</span></span>
        </a>"""
    else:
        peer_avatar = avatar_url(peer)
        peer_verified = verified_html(peer)
        header_html = f"""<a class="sender-head" href="{url_for('view_profile', username=peer.username)}"
           ...
           <span class="sender-name">{peer_verified}{html_escape_text(peer.short_name)}</span>
        </a>"""
    reply_html = ""
    if m.reply_to_id:
    ...
    if m.expires_at:
        ...
        if remain > 0:
            if remain < 60:
                label = f"{int(remain)}ث"
            elif remain < 3600:
                label = f"{int(remain//60)}د"
            elif remain < 86400:
                label = f"{int(remain//3600)}س"
            else:
                label = f"{int(remain//86400)}ي"
            disappear_badge = f'<span class="disappear-badge">⏱ {label}</span>'
        else:
            disappear_badge = '<span class="disappear-badge">⏱ انتهت</span>'

    view_once_overlay = ""
    if m.is_view_once and not is_mine:
        if m.viewed_once:
            view_once_overlay = '<div class="viewonce-overlay">👁</div>'
        else:
            view_once_overlay = '<div class="viewonce-overlay">🔥</div>'
    ...
    return f"""<div class="bubble-wrap">
      <div class="bubble {cls} clickable" id="msg-{m.id}" onclick="toggleMsgBar({m.id})" ondblclick="event.stopPropagation();doubleTap({m.id})">
        ...
        {reaction_html}{star_html}{view_once_overlay}
      </div>
...
def chat_with(username):
    ...
    if request.method == "POST":
        ...
        expires_at = None
        if disappear_sec and disappear_sec > 0:
            expires_at = now_utc_naive() + timedelta(seconds=disappear_sec)

        db.session.add(ChatMessage(
            sender_id=current_user.id, receiver_id=peer.id,
    ...
    # ✅ تعليم الرسائل "عرض مرة واحدة" كمقروءة
    view_once_msgs = ChatMessage.query.filter_by(
        sender_id=peer.id, receiver_id=current_user.id, is_view_once=True, viewed_once=False
    ).all()
    for vm in view_once_msgs:
        vm.viewed_once = True
    db.session.commit()
    ...
    qreps = QuickReply.query.filter_by(user_id=current_user.id).order_by(
        QuickReply.created_at.desc()).limit(10).all()
    qr_html = ""
    if qreps:
        qr_html = '<div class="chat-options" style="border-top:none;padding-top:4px">'
        qr_html += '<span style="font-size:11px;color:#6b7280;margin-left:4px">⚡ سريع:</span>'
        for q in qreps:
            safe = q.text.replace("\\", "\\\\").replace("'", "\\'").replace("\n", " ")
            qr_html += (f'<button type="button" onclick="fillQuick(\'{safe}\')">'
                        f'{html_escape_text(q.text[:20])}</button>')
        qr_html += '</div>'
    ...
    <div class="disappear-selector" id="disappear-selector">
      <span style="font-size:11px;color:#92400e;font-weight:700;align-self:center">⏱ اختفاء:</span>
    ...
    <div class="card">
      ...
      <div class="chat-options">
        ...
        <button type="button" onclick="muteChat()">🔕 كتم الإشعارات</button>
      </div>
    </div>
    ...
    function muteChat(){{
      fetch('/chat/{peer.username}/mute', {{
        method:'POST', credentials:'same-origin',
        headers: {{'Content-Type':'application/json'}}
      }}).then(function(r){{return r.json()}}).then(function(d){{
        if(d.ok) alert(d.muted ? '🔕 تم كتم الإشعارات' : '🔔 تم تفعيل الإشعارات');
      }}).catch(function(){{}});
    }}
...
def group_view(gid):
    ...
    for m in members:
        ...
        members_html += f"""<div class="list-item" style="cursor:default">
          ...
          <a href="{url_for('view_profile', username=u.username)}" style="flex:1;text-decoration:none;color:inherit;min-width:0">
            <div class="li-name"><span class="role-badge {role_cls}">{role_label}</span>{verified_html(u)}{u.short_name}</div>
            <div class="li-sub">@{u.username} · ID: {u.public_id}</div></a>
          <div class="li-actions">
            {copy_btn_html(u.username, "يوزر", small=True)}
            {copy_btn_html(u.public_id, "ID", small=True)}
...
def render_group_bubble(m, current_user):
    ...
    sender_verified = verified_html(sender) if sender else ""
    media_html = ""
    if m.media:
    ...
    if is_mine:
        ...
        header_html = f"""<a class="sender-head" href="{url_for('view_profile', username=sender_username)}"
           ...
           <span class="sender-name">{sender_verified}{html_escape_text(sender_name)}</span>
        </a>"""
    reply_html = ""
    if m.reply_to_id:
...
def support_dashboard():
    ...
    <div class="acct-box">
      ...
      <div class="acct-row"><span class="k">الموثّقون ✓</span>
        <span class="v" style="color:#1d9bf0;font-weight:800">{verified_count}</span></div>
        ...
        rows += f"""<label class="bulk-user-row" id="row-{u.id}">
          ...
          <label style="flex:1;margin:0">
            ...
            <div style="flex:1;min-width:0">
              <div class="bu-name">{verified_html(u)}{u.short_name}</div>
...
def support_user_detail(uid):
    ...
    if u.is_verified:
        status_badge += ' ' + verified_html(u, True)
    ...
    verify_btn = ""
    if u.is_verified:
        verify_btn = f'<a class="btn" href="{url_for("support_unverify_user", uid=u.id)}" onclick="return confirm(\'إلغاء التوثيق؟\')">✖ إلغاء التوثيق</a>'
    else:
        verify_btn = f'<a class="btn btn-primary" href="{url_for("support_verify_user", uid=u.id)}">✓ توثيق الحساب (اختيار اللون)</a>'
...
def support_verify_user(uid):
    ...
    if u.is_verified:
        ...
        <div class="privacy-note" style="background:#dcfce7;border-color:#86efac;color:#166534">
          ✅ هذا الحساب <b>موثّق حالياً</b>
          {verified_html(u, True)}
        </div>"""
    return render_page(FLASH_BLOCK + f"""
    ...
    <div style="text-align:center;margin:14px 0">
      ...
      <div class="name">{u.short_name} <span id="live-preview" class="verified-badge lg" style="background:{current_color}">{current_icon}</span></div>
    </div>
    <form method="POST" id="verify-form">
      <input type="hidden" name="action" value="verify">
      <label>اختر لون التوثيق (جميع ألوان العالم)</label>
      <div class="color-grid">{colors_html}</div>
      <input type="hidden" name="verified_color" id="verified_color" value="{current_color}">

      <label style="margin-top:16px">🎨 لون مخصص (Hex أو تدرّج)</label>
      <div style="display:flex;gap:8px">
        ...
        <input type="text" id="custom-color-text" placeholder="#RRGGBB أو linear-gradient(...)"
               value="{current_color}" style="flex:1;direction:ltr"
               oninput="document.getElementById('verified_color').value=this.value;updatePreview(this.value)">
      </div>

      <label style="margin-top:16px">⭐ اختر أيقونة التوثيق</label>
      <div class="icon-grid">{icons_html}</div>
      <input type="hidden" name="verified_icon" id="verified_icon" value="{current_icon}">

      <button type="submit" style="margin-top:16px;background:#1d9bf0">✓ توثيق الحساب</button>
    </form>
    {f'<form method="POST" style="margin-top:8px"><input type="hidden" name="action" value="unverify"><button type="submit" class="btn btn-danger" style="width:100%" onclick="return confirm(\'إلغاء التوثيق؟\')">✖ إلغاء التوثيق</button></form>' if u.is_verified else ''}
    <a class="link-center" href="{url_for('support_user_detail', uid=uid)}">← رجوع</a>
    </div>
    <style>
    .color-grid{{display:grid;grid-template-columns:repeat(auto-fill,minmax(100px,1fr));gap:8px;max-height:400px;overflow-y:auto;padding:8px;background:#f9fafb;border-radius:10px;border:1px solid var(--border)}}
    .color-opt{{display:flex;flex-direction:column;align-items:center;gap:4px;padding:8px;border-radius:10px;cursor:pointer;border:2px solid transparent;transition:.15s;background:#fff}}
    .color-opt:hover{{border-color:#93c5fd;transform:translateY(-2px)}}
    .color-opt.selected{{border-color:#2563eb;background:#eff6ff;box-shadow:0 0 0 3px rgba(37,99,235,.15)}}
    ...
    .icon-grid{{display:grid;grid-template-columns:repeat(auto-fill,minmax(50px,1fr));gap:6px;padding:8px;background:#f9fafb;border-radius:10px;border:1px solid var(--border)}}
    .icon-opt{{background:#fff;border:2px solid transparent;border-radius:10px;padding:8px;font-size:22px;cursor:pointer;transition:.15s;width:auto;margin:0}}
    .icon-opt:hover{{border-color:#93c5fd;transform:scale(1.1)}}
    .icon-opt.selected{{border-color:#2563eb;background:#eff6ff}}
    </style>
    <script>
    ...
    function selectIcon(icon, el){{
      ...
      document.getElementById('live-preview').textContent = icon;
    }}
...
def support_users():
    ...
    for u in users:
        ...
        if u.is_verified:
            verify_btn = f'<a class="btn btn-sm" href="{url_for("support_verify_user", uid=u.id)}" title="تعديل التوثيق">🎨</a>'
        else:
            verify_btn = f'<a class="btn btn-sm btn-primary" href="{url_for("support_verify_user", uid=u.id)}" title="توثيق">✓</a>'
        rows += f"""<div class="list-item" style="cursor:default">
          <img class="avatar-sm" src="{avatar_url(u)}">
          <div style="flex:1;min-width:0">
            <div class="li-name">{verify_mark}{u.short_name} {status}</div>
