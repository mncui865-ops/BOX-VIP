...
"""
╔══════════════════════════════════════════════════════════╗
║              𝙵𝙱𝙸 𝚂𝚄𝙳𝙰𝙽𝙴𝚂𝙴  —  v4.0 (Railway Ready)      ║
╠══════════════════════════════════════════════════════════╣
║  ✅ حالات عامة + دردشة مباشرة من الحالة                  ║
║  ✅ مجموعات كاملة (مالك/مشرف/طرد/إضافة/بحث)             ║
║  ✅ تحديث تلقائي للرسائل                                  ║
║  ✅ مسار دعم محمي بكلمة سر                                ║
║  ✅ أزرار فيسبوك وتلغرام                                  ║
╚══════════════════════════════════════════════════════════╝
"""
...
if VOLUME_MOUNT and os.path.isdir(VOLUME_MOUNT):
    ...
    for sub in ["uploads", "uploads/avatars", "uploads/statuses", "uploads/chats", "uploads/groups"]:
...
class Status(db.Model):
    ...
    privacy = db.Column(db.String(10), default="public")  # دائماً public الآن
...
def group_avatar_url(g):
    if g.avatar: return url_for("serve_upload", subpath=f"groups/{g.avatar}")
...
def before_each_request():
    ...
    allowed = {"set_nickname","logout","static","serve_upload","welcome","support_login"}
...
def badge_html():
    if current_user.is_authenticated:
        ...
        if c: return f'<span class="badge" id="chat-badge-main">{c}</span>'
...
def login():
    ...
    <div class="social-box">
      ...
      <div class="social-btns">
        <a class="social-btn tg" href="https://t.me/Zero_Vib" target="_blank" rel="noopener">
          <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
            <path d="M9.78 18.65l.28-4.23 7.68-6.92c.34-.31-.07-.46-.52-.19L7.74 13.3 3.64 12c-.88-.25-.89-.86.2-1.3l15.97-6.16c.73-.33 1.43.18 1.15 1.3l-2.72 12.81c-.19.91-.74 1.13-1.5.71L12.6 16.3l-1.99 1.93c-.23.23-.42.42-.83.42z"/>
          </svg>
          تلغرام
        </a>
        <a class="social-btn fb" href="https://www.facebook.com/profile.php?id=100084033551388" target="_blank" rel="noopener">
          <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
...
def friend_request(username):
    ...
    if not target:
        flash("المستخدم غير موجود.", "error"); return redirect(url_for("discover"))
    if target.id == current_user.id:
        flash("لا يمكنك إضافة نفسك.", "error"); return redirect(url_for("profile_me"))
    if target.is_banned:
        flash("لا يمكن إضافة هذا المستخدم.", "error"); return redirect(url_for("discover"))
...
def block_user(username):
    ...
    if not target or target.id == current_user.id:
        flash("لا يمكن حظر هذا المستخدم.","error"); return redirect(url_for("search"))
...
def unblock_user(username):
    target = User.query.filter_by(username=username.lower().lstrip("@")).first()
    if not target:
        flash("المستخدم غير موجود.","error"); return redirect(url_for("blocked_list"))
    b = Block.query.filter_by(blocker_id=current_user.id, blocked_id=target.id).first()
    if not b:
        flash("هذا المستخدم غير محظور.","error"); return redirect(url_for("blocked_list"))
...
def archive_chat(username):
    target = User.query.filter_by(username=username.lower().lstrip("@")).first()
    if not target or target.id == current_user.id:
        flash("لا يمكن أرشفة هذه المحادثة.","error"); return redirect(url_for("chats"))
    existing = ArchivedChat.query.filter_by(user_id=current_user.id, peer_id=target.id).first()
    if existing:
        flash("المحادثة مؤرشفة بالفعل.","error"); return redirect(request.referrer or url_for("chats"))
...
def unarchive_chat(username):
    target = User.query.filter_by(username=username.lower().lstrip("@")).first()
    if not target:
        flash("المستخدم غير موجود.","error"); return redirect(url_for("archived_list"))
    a = ArchivedChat.query.filter_by(user_id=current_user.id, peer_id=target.id).first()
    if not a:
        flash("المحادثة غير مؤرشفة.","error"); return redirect(url_for("archived_list"))
...
def delete_account():
    if request.method == "POST":
        ...
        if confirm_text != "DELETE":
            flash("اكتب كلمة DELETE للتأكيد.","error"); return redirect(url_for("delete_account"))
        uid = current_user.id
        try:
            if current_user.avatar:
                try:
            ...
            db.session.rollback()
            flash(f"حدث خطأ: {e}","error"); return redirect(url_for("delete_account"))
        logout_user()
        resp = make_response(redirect(url_for("login")))
        resp.delete_cookie("device_token")
...
def status_viewers(sid):
    ...
    if not rows: rows = '<p class="empty">لا أحد شاهد حالتك بعد.</p>'
    return render_page(FLASH_BLOCK + f"""
    <div class="card">
...
# ═══════════════════ الصفحة الرئيسية (الحالات العامة) ═══════════════════
@app.route("/feed")
@login_required
def feed():
    ...
    # الحالات الآن عامة دائماً - لا فلترة بالخصوصية
    ...
    # تسجيل المشاهدات
    for s in statuses:
        if s.user_id == current_user.id: continue
        if s.show_viewers:
    ...
    for uid, items in by_user.items():
        for idx, s in enumerate(items):
            ...
            # 🆕 أزرار أسفل الحالة (دردشة مباشرة + إضافة صديق)
            bottom_actions = ""
            if s.user_id != current_user.id:
...
def chats():
    ...
    if not rows: rows = '<p class="empty">لا محادثات. أضف أصدقاء لبدء الدردشة.</p>'
...
def chat_with(username):
    ...
    if is_blocked_between(current_user.id, peer.id):
        flash("لا يمكن إتمام العملية (يوجد حظر).", "error"); return redirect(url_for("chats"))
    ...
    if request.args.get("ajax") == "1":
        ...
        for m in msgs:
            ...
            bubbles += f"""<div class="bubble {cls}">{media_html}{body_html}<span class="t">{fmt_sd(m.created_at, "%H:%M")}</span></div>"""
        return jsonify({"html": bubbles, "unread": unread_chat_count(current_user.id)})
...
def groups_list():
    ...
    if not my_html: my_html = '<p class="empty">لا مجموعات. أنشئ واحدة!</p>'

    all_html = ""
    for g in all_groups:
        if g.id in {m.id for m in my_groups}: continue
...
def group_create():
    if request.method == "POST":
        ...
        if len(name) < 2 or len(name) > 64:
            flash("اسم المجموعة: 2-64 حرف.", "error"); return redirect(url_for("group_create"))
        if len(description) > 300:
            flash("الوصف طويل جداً.", "error"); return redirect(url_for("group_create"))
...
def group_view(gid):
    ...
    if g.is_banned:
        flash("هذه المجموعة محظورة.", "error"); return redirect(url_for("groups_list"))
...
def group_join(gid):
    ...
    if get_group_member(gid, current_user.id):
        flash("أنت عضو بالفعل.", "error"); return redirect(url_for("group_view", gid=gid))
...
def group_kick(gid, uid):
    ...
    if not target:
        flash("العضو غير موجود.", "error"); return redirect(url_for("group_view", gid=gid))
    if target.role == "owner":
        flash("لا يمكن طرد المالك.", "error"); return redirect(url_for("group_view", gid=gid))
    if me.role == "admin" and target.role == "admin":
        flash("لا يمكن للمشرف طرد مشرف آخر.", "error"); return redirect(url_for("group_view", gid=gid))
...
def group_add_member(gid):
    ...
    if not target:
        flash("المستخدم غير موجود.", "error"); return redirect(url_for("group_chat", gid=gid))
    if target.is_banned:
        flash("لا يمكن إضافة مستخدم محظور.", "error"); return redirect(url_for("group_chat", gid=gid))
    if get_group_member(gid, target.id):
        flash("المستخدم عضو بالفعل.", "error"); return redirect(url_for("group_chat", gid=gid))
...
def group_chat(gid):
    ...
    if request.args.get("ajax") == "1":
        ...
        return jsonify({"html": bubbles, "unread": 0})
    ...
    admin_link = f'<a class="btn btn-sm" href="{url_for("group_view", gid=gid)}">👥 الأعضاء ({member_count})</a>' if is_admin else f'<a class="btn btn-sm" href="{url_for("group_view", gid=gid)}">👥 الأعضاء</a>'
    add_form = ""
    if is_admin:
...
def group_report(gid):
    g = Group.query.get_or_404(gid)
    if g.owner_id == current_user.id:
        flash("لا يمكنك إبلاغ مجموعتك.", "error"); return redirect(url_for("group_view", gid=gid))
    if request.method == "POST":
...
def inbox():
    ...
    if not rows: rows = '<p class="empty">لا رسائل مجهولة.</p>'
    return render_page(FLASH_BLOCK + topbar_html(badge) + f"""
...
def support_dashboard():
    ...
    <div class="card">
      ...
      <div class="actions">
        <a class="btn btn-primary" href="{url_for('support_reports')}">📋 بلاغات المستخدمين</a>
        <a class="btn btn-primary" href="{url_for('support_group_reports')}">📋 بلاغات المجموعات</a>
...
def support_reports():
    ...
    for r in reports:
        ...
        rows += f"""<div class="list-item" style="cursor:default;flex-wrap:wrap">
          <div style="flex:1;min-width:200px">
            <div class="li-name">🎯 @{target.username} ({target.short_name})</div>
            <div class="li-sub">من: {('@'+reporter.username) if reporter else 'مجهول'} · {fmt_sd(r.created_at)}</div>
            <div style="font-size:13px;margin-top:6px;color:#374151">{r.reason}</div>
            <div class="li-sub">إجمالي بلاغات الهدف: {target.reports_count}</div>
          </div>
          <div class="li-actions">
            <a class="btn btn-sm" href="{url_for('view_profile', username=target.username)}">عرض</a>
            <a class="btn btn-sm btn-danger" href="{url_for('support_ban_user', uid=target.id)}"
               onclick="return confirm('حظر هذا المستخدم نهائياً؟')">🚫 حظر</a>
    ...
    return render_page(FLASH_BLOCK + f"""
    <div class="card">
      <h2>📋 بلاغات المستخدمين ({len(reports)})</h2>
      {rows}
      <a class="link-center" href="{url_for('support_dashboard')}">← رجوع</a>
    </div>""", title="البلاغات")

@app.route("/support/group-reports")
@support_required
def support_group_reports():
    reports = GroupReport.query.order_by(GroupReport.created_at.desc()).all()
    rows = ""
    for r in reports:
        reporter = User.query.get(r.reporter_id) if r.reporter_id else None
        ...
        rows += f"""<div class="list-item" style="cursor:default;flex-wrap:wrap">
          <div style="flex:1;min-width:200px">
            <div class="li-name">◉ {g.name} (@{g.public_id})</div>
            <div class="li-sub">من: {('@'+reporter.username) if reporter else 'مجهول'} · {fmt_sd(r.created_at)}</div>
            <div style="font-size:13px;margin-top:6px;color:#374151">{r.reason}</div>
          </div>
          <div class="li-actions">
            <a class="btn btn-sm" href="{url_for('group_view', gid=g.id)}">عرض</a>
            <a class="btn btn-sm btn-danger" href="{url_for('support_ban_group', gid=g.id)}"
               onclick="return confirm('حظر هذه المجموعة؟')">🚫 حظر</a>
    ...
    return render_page(FLASH_BLOCK + f"""
    <div class="card">
      <h2>📋 بلاغات المجموعات ({len(reports)})</h2>
      {rows}
      <a class="link-center" href="{url_for('support_dashboard')}">← رجوع</a>
    </div>""", title="بلاغات المجموعات")
...
def support_ban_group(gid):
    ...
    return redirect(request.referrer or url_for("support_group_reports"))
...
def support_dismiss_group_report(rid):
    ...
    db.session.delete(r); db.session.commit()
    flash("تم تجاهل البلاغ.", "success")
    return redirect(url_for("support_group_reports"))
