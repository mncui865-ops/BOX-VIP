# -*- coding: utf-8 -*-
"""
FBI SUDANESE — v18-Simple (بدون منشورات)
"""
import os, re, uuid, random, string, secrets, hashlib, time, traceback, json, math, colorsys
...
GIFTS_DIR = os.path.join(UPLOAD_DIR, "gifts")
for d in (AVATAR_DIR, STATUS_DIR, CHAT_DIR, GROUP_DIR, COVER_DIR, GIFTS_DIR):
...
app.config["GIFTS_FOLDER"] = GIFTS_DIR
...
# ═══════════════════════════════════════════════════════════════
# MODELS (بدون Post/PostReaction/PostComment)
# ═══════════════════════════════════════════════════════════════
class User(UserMixin, db.Model):
    ...
    total_gifts_received = db.Column(db.Integer, default=0)
    total_gifts_sent = db.Column(db.Integer, default=0)
...
class Gift(db.Model):
    ...
    gift_type = db.Column(db.String(20), default="star")
...
def process_auto_ban(user):
    ...
    try:
        ...
        for m in ChatMessage.query.filter(or_(ChatMessage.sender_id == uid, ChatMessage.receiver_id == uid)).all():
            ...
            db.session.delete(m)
        Gift.query.filter(or_(Gift.sender_id == uid, Gift.receiver_id == uid)).delete(synchronize_session=False)
...
GLOW_ICONS = {
    ...
    "coffee": '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M20 8h-2V5a1 1 0 0 0-1-1H3a1 1 0 0 0-1 1v10a3 3 0 0 0 3 3h10a3 3 0 0 0 3-3v-1h2a2 2 0 0 0 2-2V10a2 2 0 0 0-2-2zm0 4h-2V10h2v2z"/></svg>',
    "music": '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 3v10.55A4 4 0 1 0 14 17V7h4V3h-6z"/></svg>',
    "fireworks": '<svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 2l2 5 5-2-2 5 5 2-5 2 2 5-5-2-2 5-2-5-5 2 2-5-5-2 5-2-2-5 5 2z"/></svg>',
...
def glow_reaction_icon(name):
    ...
    # ═══════════════════════════════════════════════════════════════
# VERIFY_COLORS — 60 أساسي + 240 مولّد = 300+
# ═══════════════════════════════════════════════════════════════
VERIFY_COLORS_BASE = [
...
# ═══════════════════════════════════════════════════════════════
# ALL_VERIFY_SHAPES — 360 شكل SVG (بدون إيموجي)
# ═══════════════════════════════════════════════════════════════
...
for i in range(312):
    ...
    for s in range(sides):
        ...
        x = 12 + radius * 2.5 * math.cos(a)
        y = 12 + radius * 2.5 * math.sin(a)
        pts.append(f"{x:.1f},{y:.1f}")
    path = "M" + " L".join(pts) + " Z"
    ALL_VERIFY_SHAPES.append((f"poly_{i}", path))


def verified_html(user, large=False):
    """شكل توثيق موحّد — SVG + تلوين + شكل مخصص"""
...
def unread_gifts_count(user_id):
    return Gift.query.filter_by(receiver_id=user_id, is_read=False).count()
...
.gift-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(80px,1fr));gap:10px;margin-top:10px}
.gift-opt{background:#fff;border:2px solid var(--border);border-radius:14px;padding:12px;
  cursor:pointer;transition:.15s;text-align:center;}
.gift-opt:hover{border-color:#fbbf24;transform:translateY(-3px);box-shadow:0 4px 12px rgba(251,191,36,.2)}
.gift-opt.selected{border-color:#f59e0b;background:#fffbeb;box-shadow:0 0 0 3px rgba(245,158,11,.15)}
.gift-opt .g-icon{width:42px;height:42px;margin:0 auto 6px;display:flex;align-items:center;justify-content:center;
  filter:drop-shadow(0 0 6px currentColor);}
.gift-opt .g-icon svg{width:100%;height:100%;}
.gift-opt .g-name{font-size:11px;font-weight:700;color:#374151}
.gift-card{background:linear-gradient(135deg,#fef3c7,#fed7aa);border:2px solid #fbbf24;
  border-radius:14px;padding:16px;text-align:center;margin-bottom:12px;}
body.dark .gift-card{background:linear-gradient(135deg,#422006,#713f12);border-color:#92400e;}
.gift-card .gc-icon{width:64px;height:64px;margin:0 auto 8px;display:flex;align-items:center;justify-content:center;
  filter:drop-shadow(0 0 10px currentColor) drop-shadow(0 0 20px currentColor);
  animation:giftBounce 1.5s ease-in-out infinite;}
.gift-card .gc-icon svg{width:100%;height:100%;}
@keyframes giftBounce{0%,100%{transform:translateY(0) scale(1);}50%{transform:translateY(-6px) scale(1.06);}}
...
def render_page(body, title=None, **ctx):
    ...
    return render_template_string(
        ...
        "function fireConfetti(){var colors=['#25D366','#FFD700','#FF6B6B','#4ECDC4','#DDA0DD'];"
        "for(var i=0;i<30;i++){var c=document.createElement('div');c.className='confetti';"
        ...
        "function playNotificationSound(){try{"
        ...
        "o2.frequency.value=1320;o2.type='sine';g2.gain.value=0.08;"
        "o2.start(a.currentTime+0.08);o2.stop(a.currentTime+0.25);"
        "}catch(e){}}"
        ...
        "var _chatRefreshTimer=null;var _lastMsgCount=0;"
        ...
        "if(d.msg_count!==undefined&&_lastMsgCount>0&&d.msg_count>_lastMsgCount){"
        "if(document.body.dataset.sound!=='0')playNotificationSound();}"
        "if(d.msg_count!==undefined)_lastMsgCount=d.msg_count;"
...
def badge_html():
    if current_user.is_authenticated:
        ...
        g = unread_gifts_count(current_user.id)
        total = c + g
        if total:
            return f'<span class="badge pulse" id="chat-badge-main">{total}</span>'
...
def view_profile(username):
    ...
    mood_bubble = ""
    if user.mood_text:
        mood_bubble = f"""<div class="mood-bubble-wrap"><div class="mood-bubble">
          <span class="mood-icon">{glow_icon("mood",18,"#f59e0b")}</span>
    ...
    if cu:
        if is_own:
            ...
            cover_html = f'<div class="profile-cover-wrap">{mood_bubble}<img src="{cu}" onclick="openAvatar(\'{cu}\')"></div>'
    else:
        if is_own:
            ...
            cover_html = f'<div class="profile-cover-wrap no-cover" style="background:linear-gradient(135deg,#667ba2,#764ba2)">{mood_bubble}</div>'
    ...
    name_block = f"""<div class="profile-name-block">
      <div class="pn-verified">{verified_big}</div>
      <div class="pn-text">{html_escape_text(user.short_name)}</div>
    </div>"""
    ...
    <div class="stats-grid" style="margin-top:14px">
      ...
      <div class="stat-card"><div class="sc-value">{user.total_gifts_received or 0}</div><div class="sc-label">🎁 هدية</div></div>
    </div>"""
    actions = ""
    if is_own:
        actions = f"""
...
def edit_extras():
    ...
    <form method="POST">
      <label>💬 الحالة المزاجية (تظهر كفقاعة فوق غلافك)</label>
      ...
      <label>❤️ الإيموجيات المفضلة (أسماء SVG)</label>
...
def render_bubble_chat(m, current_user, peer):
    ...
    if m.reaction:
        icon_map = {"heart": "heart", "thumb": "thumb", "laugh": "laugh", "wow": "wow", "sad": "sad", "pray": "pray",
                    "❤️": "heart", "👍": "thumb", "😂": "laugh", "😮": "wow", "😢": "sad", "🙏": "pray"}
...
def chat_with(username):
    ...
    <div class="card">
      <div class="chat-header">
        ...
        <div style="margin-right:auto;display:flex;gap:6px;align-items:center;flex-wrap:wrap">
          ...
          {arch_action}
          <button type="button" class="btn btn-sm" onclick="toggleSearch()">{glow_icon("search",14)}</button>
          ...
          <button type="button" class="btn btn-sm" style="background:#fef2f2;color:#991b1b;border-color:#fecaca"
                  onclick="clearChatConfirm()">{glow_icon("trash",14,"#991b1b")}</button>
        </div>
      </div>
    ...
    function editMsg(mid, oldBody){{
      var newBody = prompt('✏️ تعديل الرسالة:', oldBody);
...
def group_chat(gid):
    ...
    function editMsgG(mid, oldBody){{
      var newBody = prompt('✏️ تعديل:', oldBody);
      if(newBody===null || newBody.trim()==='') return;
...
def support_chat_reports():
    ...
    for r in reports:
        ...
        actions = ""
        if r.status == "pending":
            actions = f"""<a class="btn btn-sm btn-danger" href="{url_for('support_review_chat_report', rid=r.id, decision='upheld')}" onclick="return confirm('حظر دائم؟')">🚫</a>
            <a class="btn btn-sm btn-primary" href="{url_for('support_review_chat_report', rid=r.id, decision='dismissed')}">✅</a>"""
        else:
            undo = 'dismissed' if r.status == 'upheld' else 'upheld'
            actions = f'<a class="btn btn-sm" href="{url_for("support_review_chat_report", rid=r.id, decision=undo)}">↩</a>'
        ...
        rows += f"""<div class="list-item" style="cursor:default;flex-wrap:wrap;border-right:4px solid #f59e0b">
          ...
          <div style="flex:1;min-width:240px">
            ...
            <div class="li-sub">🪪 {reporter_html}</div>
          ...
          <div class="li-actions" style="flex-direction:column;gap:4px">
            <a class="btn btn-sm" href="{url_for('support_user_detail', uid=tgt.id)}">👁</a>
            {actions}
          </div></div>"""
...
def support_verify_user(uid):
    ...
    shapes_html = ""
    for k, path in ALL_VERIFY_SHAPES[:120]:
...
def support_users():
    ...
    for u in users:
        ...
        action = (f'<a class="btn btn-sm btn-primary" href="{url_for("support_unban_user", uid=u.id)}">✅</a>'
                  if u.is_banned else
                  f'<a class="btn btn-sm btn-danger" href="{url_for("support_ban_user", uid=u.id)}" onclick="return confirm(\'حظر دائم؟\')">🚫</a>')
