# -*- coding: utf-8 -*-
"""
FBI SUDANESE — v10.0 (WhatsApp Killer Edition)
- ✅ بروفايل المرسل بجانب كل رسالة (فردي + جماعي) — يظهر حتى لرسائلك
- ✅ مؤشر "يكتب..." في الفردي والجماعي
- ✅ زر حذف الرسائل + إزالة التفاعل
- ✅ إرسال الأغاني (mp3, m4a, wav, flac...)
- ✅ تحديث تلقائي كل ثانية
- ✅ حظر جماعي متعدد
- ✅ خلفية موحدة لجميع الدردشات (يتحكم فيها الدعم)
- ✅ علامة توثيق زرقاء ✓✓ (يمنحها الدعم)
- ✅ زر وضع ليلي بدائرتين (بيضاء/سوداء) بدون هلال
...
if VOLUME_MOUNT and os.path.isdir(VOLUME_MOUNT):
    ...
    for sub in ["uploads", "uploads/avatars", "uploads/statuses",
                "uploads/chats", "uploads/groups", "uploads/chatbg"]:
...
CHATBG_DIR = os.path.join(UPLOAD_DIR, "chatbg")
for d in (AVATAR_DIR, STATUS_DIR, CHAT_DIR, GROUP_DIR, CHATBG_DIR):
...
app.config["CHATBG_FOLDER"] = CHATBG_DIR
...
class User(UserMixin, db.Model):
    ...
    is_verified = db.Column(db.Boolean, default=False)  # ✅ توثيق
...
class SiteSettings(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    chat_bg_image = db.Column(db.String(300), default="")  # ✅ خلفية الدردشات
    ...
    with app.app_context():
        ...
        try:
            ...
            if "privacy" not in status_cols:
                ...
                db.session.commit()
            site_cols = [c["name"] for c in insp.get_columns("site_settings")]
            if "chat_bg_image" not in site_cols:
                db.session.execute(text('ALTER TABLE "site_settings" ADD COLUMN chat_bg_image VARCHAR(300) DEFAULT \'\''))
                db.session.commit()
            for tbl, cols in [
...
def verified_html(user, large=False):
    ...
    if user and getattr(user, "is_verified", False):
        ...
        return f'<span class="{cls}" title="حساب موثّق">✓</span>'
    return ""
...
.verified-badge{display:inline-flex;align-items:center;justify-content:center;
  width:15px;height:15px;border-radius:50%;background:#1d9bf0;color:#fff;
  font-size:10px;margin-right:3px;flex-shrink:0;
  box-shadow:0 0 0 1.5px var(--surface);vertical-align:middle;font-weight:900;line-height:1;}
.verified-badge.lg{width:20px;height:20px;font-size:14px;}
...
def before_each_request():
    ...
    allowed_special = {
        ...
        "support_chat_bg", "support_chat_bg_upload", "support_chat_bg_delete",
...
def friends_list():
    ...
    for f in friends:
        ...
        <a href="{url_for('view_profile', username=f.username)}" style="flex:1;text-decoration:none;color:inherit">
          <div class="li-name">{online}{verified_html(f)}{f.short_name}</div>
    ...
    for u in pagination.items:
        ...
        rows += f"""<div class="list-item" style="cursor:default">
          <a href="{url_for('view_profile', username=u.username)}" style="display:flex;align-items:center;gap:10px;flex:1;text-decoration:none;color:inherit;min-width:0">
            ...
            <div style="flex:1;min-width:0"><div class="li-name">{online}{verified_html(u)}{u.short_name}</div>
...
def search():
    ...
    for u in users_results:
        ...
        <div class="li-name">{online}{verified_html(u)}{u.short_name}</div>
        <div class="li-sub">@{u.username} · ID: {u.public_id}</div></a>
        <div class="li-actions">
          {copy_btn_html(u.username, "يوزر", small=True)}
          {copy_btn_html(u.public_id, "ID", small=True)}
    ...
    for pinned, _, f, last_msg, unread, muted in items:
        ...
        rows += f"""<div class="list-item">
          ...
          <a href="{url_for('chat_with', username=f.username)}" style="flex:1;text-decoration:none;color:inherit;min-width:0">
            <div class="li-name">{pin_icon}{mute_icon}{online}{verified_html(f)}{f.short_name} {unread_tag}</div>
...
def render_bubble_chat(m, current_user, peer):
    ...
    # ✅ صورة المرسل + الاسم — تظهر للجميع (حتى أنت)
    header_html = ""
    if is_mine:
...
def chat_with(username):
    ...
    bg_style = ""
    if s.chat_bg_image:
        bg_url = url_for("serve_upload", subpath=f"chatbg/{s.chat_bg_image}")
...
def group_view(gid):
    ...
    for m in members:
        ...
        members_html += f"""<div class="list-item" style="cursor:default">
          <img class="avatar-sm" src="{avatar_url(u)}" onclick="openAvatar('{avatar_url(u)}')">
          <a href="{url_for('view_profile', username=u.username)}" style="flex:1;text-decoration:none;color:inherit;min-width:0">
            <div class="li-name">{online}<span class="role-badge {role_cls}">{role_label}</span>{verified_html(u)}{u.short_name}</div>
            <div class="li-sub">@{u.username} · ID: {u.public_id}</div></a>
          <div class="li-actions">
            {copy_btn_html(u.username, "يوزر", small=True)}
            {copy_btn_html(u.public_id, "ID", small=True)}
    ...
    if m.media:
        ...
        if m.media_type == "video":
            ...
            media_html = (f'<img src="{url}" style="max-width:100%;border-radius:10px;'
                          ...
                          f'onclick="event.stopPropagation();{img_onclick}">')
    body_html = f'<div>{linkify_text(m.body)}</div>' if m.body else ""
    # ✅ صورة + اسم المرسل (للجميع: أنت + الآخرين)
    header_html = ""
    if is_mine:
        my_avatar = avatar_url(current_user)
        my_verified = verified_html(current_user)
...
def group_chat(gid):
    ...
    <div class="card">
      ...
      <div class="chat-box" id="chatbox" style="margin-top:10px" {bg_style}>{bubbles}</div>
      <div id="reply-preview" class="reply-preview" style="display:none">
        <div class="rp-info">
          <div class="rp-name" id="rp-name"></div>
          <div class="rp-body" id="rp-body"></div>
...
def support_dashboard():
    ...
    s = get_site_settings()
    bg_status = "🟢 مفعّلة" if s.chat_bg_image else "⚪ لا توجد"
    return render_page(FLASH_BLOCK + f"""
    <div class="card"><h2>🛡 لوحة الدعم</h2>
    <div class="acct-box">
      ...
      <div class="acct-row"><span class="k">خلفية الدردشات</span>
        <span class="v" style="font-family:inherit">{bg_status}</span></div>
    ...
    <div class="actions">
      ...
      <a class="btn" href="{url_for('support_chat_bg')}">🖼 خلفية الدردشات</a>
...
# ✅ خلفية الدردشات
@app.route("/support/chat-bg", methods=["GET", "POST"])
@support_required
def support_chat_bg():
    s = get_site_settings()
    if request.method == "POST":
        ...
        return redirect(url_for("support_chat_bg"))
    preview = ""
    if s.chat_bg_image:
        img_url = url_for("serve_upload", subpath=f"chatbg/{s.chat_bg_image}")
        preview = f'<img src="{img_url}" class="splash-preview" style="max-width:320px">'
        delete_btn = f"""
        <form method="POST" action="{url_for('support_chat_bg_delete')}" style="margin-top:8px"
              onsubmit="return confirm('🗑 حذف خلفية الدردشات؟')">
          <button type="submit" class="btn btn-danger" style="width:100%">🗑 حذف الخلفية الحالية</button>
        </form>"""
    else:
        preview = '<p class="empty" style="margin:14px 0">لا توجد خلفية بعد.</p>'
        delete_btn = ""
    return render_page(FLASH_BLOCK + f"""
    <div class="card"><h2>🖼 خلفية الدردشات الموحدة</h2>
    <div class="privacy-note">
      صورة تظهر خلف <b>جميع المحادثات</b> (الفردية والجماعية) لجميع المستخدمين.
    </div>
    {preview}
    {delete_btn}
    <form method="POST" action="{url_for('support_chat_bg_upload')}" enctype="multipart/form-data" style="margin-top:14px">
      <label>رفع صورة جديدة (ستُفعّل تلقائياً)</label>
      <input type="file" name="bg_image" accept="image/*" required>
      <button type="submit" class="btn btn-primary" style="margin-top:10px">📤 رفع وتفعيل</button>
    </form>
    <a class="link-center" href="{url_for('support_dashboard')}">← رجوع للوحة</a>
    </div>""", title="خلفية الدردشات")


@app.route("/support/chat-bg/upload", methods=["POST"])
@support_required
def support_chat_bg_upload():
    ...
    if s.chat_bg_image:
        try:
            old = os.path.join(app.config["CHATBG_FOLDER"], s.chat_bg_image)
            if os.path.exists(old): os.remove(old)
        except OSError:
            pass
    ext = file.filename.rsplit(".", 1)[1].lower()
    fn = f"{uuid.uuid4().hex}.{ext}"
    file.save(os.path.join(app.config["CHATBG_FOLDER"], fn))
    s.chat_bg_image = fn
    s.updated_at = now_utc_naive()
    db.session.commit()
    flash("✅ تم رفع الخلفية وتفعيلها لجميع الدردشات.", "success")
    return redirect(url_for("support_chat_bg"))


@app.route("/support/chat-bg/delete", methods=["POST"])
@support_required
def support_chat_bg_delete():
    s = get_site_settings()
    if s.chat_bg_image:
        try:
            p = os.path.join(app.config["CHATBG_FOLDER"], s.chat_bg_image)
            if os.path.exists(p): os.remove(p)
        except OSError:
            pass
        s.chat_bg_image = ""
        s.updated_at = now_utc_naive()
        db.session.commit()
        flash("تم حذف خلفية الدردشات.", "success")
    else:
        flash("لا توجد خلفية لحذفها.", "info")
    return redirect(url_for("support_chat_bg"))
...
def support_user_detail(uid):
    ...
    if u.is_verified:
        status_badge += ' <span class="verified-badge lg" title="موثّق">✓</span>'
    ...
    if u.is_verified:
        ...
        verify_btn = f'<a class="btn btn-primary" href="{url_for("support_verify_user", uid=u.id)}" onclick="return confirm(\'توثيق هذا الحساب؟\')">✓ توثيق الحساب</a>'
...
@app.route("/support/verify/<int:uid>")
@support_required
...
def support_users():
    ...
    for u in users:
        ...
        if u.is_verified:
            verify_btn = f'<a class="btn btn-sm" href="{url_for("support_unverify_user", uid=u.id)}" title="إلغاء التوثيق">✖</a>'
        else:
