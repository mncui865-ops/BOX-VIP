# -*- coding: utf-8 -*-
"""
FBI SUDANESE — v6.3 (Railway Ready)
- 4 أزرار تواصل بنفس التنسيق: Telegram×2 + Facebook×2
- ID القروب بارز مع زر نسخ
- طلبات انضمام للقروبات + قبول/رفض
- نقل ملكية القروب تلقائياً عند مغادرة المالك
- صفحة حظر مخصصة (تسجيل خروج + حساب جديد فقط)
- بلاغات تعرض صاحب الشوكة + إمكانية حظره
- ✅ جديد v6.3:
  * الرد على الرسائل (Reply) في الفردي والجماعي مثل واتساب
  * التفاعل بالإيموجي على أي رسالة
  * صورة المجموعة: الكل يفتحها كاملة، فقط المالك/المشرف يغيّرها
  * لوحة الدعم: عرض الرسائل المبلّغ عنها + عدد بلاغات الحساب + نشاطه
"""
...
def process_auto_ban(user):
    ...
    chat_msgs = ChatMessage.query.filter(
        or_(ChatMessage.sender_id == user.id, ChatMessage.receiver_id == user.id)).all()
    for m in chat_msgs:
        if m.media:
            try:
    ...
    Friendship.query.filter(or_(Friendship.requester_id == user.id,
                                Friendship.addressee_id == user.id)).delete(synchronize_session=False)
    Block.query.filter(or_(Block.blocker_id == user.id,
                           Block.blocked_id == user.id)).delete(synchronize_session=False)
    ArchivedChat.query.filter(or_(ArchivedChat.user_id == user.id,
                                  ArchivedChat.peer_id == user.id)).delete(synchronize_session=False)
    SavedChat.query.filter(or_(SavedChat.user_id == user.id,
                               SavedChat.peer_id == user.id)).delete(synchronize_session=False)
...
<div class="card center" style="border:2px solid #fecaca;background:#fef2f2">
  ...
  <div style="background:#fff;border:1px solid #fecaca;border-radius:12px;
    ...
    <p><b>السبب:</b> تم حظر حسابك بسبب مخالفة شروط الاستخدام أو تجاوز عدد البلاغات المسموح.</p>
...
def before_each_request():
    ...
    if request.endpoint not in allowed_banned_check:
...
.container{width:100%;max-width:560px}.brand{text-align:center;margin-bottom:26px}
...
.card.center{text-align:center}h2{font-size:17px;font-weight:700;text-align:center;margin-bottom:18px}
...
button:hover{background:var(--accent-hover)}a{color:var(--text);text-decoration:none}a:hover{text-decoration:underline}
a.link-center{display:block;text-align:center;margin-top:14px;font-size:14px;color:var(--muted)}
a.link-center:hover{color:var(--text);text-decoration:none}.pw-wrap{position:relative}
.pw-wrap input{padding-left:46px}.pw-toggle{position:absolute;left:8px;top:50%;transform:translateY(-50%);
...
.avatar-sm:hover{box-shadow:0 0 0 2px var(--blue)}.name{font-size:20px;font-weight:800}
...
.btn-danger:hover{background:#fee2e2}.btn-disabled{background:#e5e7eb;color:#9ca3af;cursor:not-allowed;border-color:#d1d5db}
.btn-disabled:hover{background:#e5e7eb;text-decoration:none}.btn-sm{padding:8px 12px;font-size:13px;width:auto;display:inline-block;margin:0}
...
.acct-row{display:flex;justify-content:space-between;align-items:center;padding:6px 0;font-size:14px;}
.acct-row .k{color:#6b7280;font-size:12px;text-transform:uppercase;letter-spacing:.5px}
.acct-row .v{font-family:monospace;font-weight:700;color:#1e3a8a;font-size:13px;display:flex;align-items:center;gap:6px}
.copy-btn{background:#fff;border:1px solid #c7d2fe;color:#3730a3;padding:4px 10px;border-radius:8px;
font-size:12px;cursor:pointer;margin:0;width:auto;font-weight:600;transition:.15s;}
...
font-size:13px;font-weight:600;text-decoration:none;cursor:pointer;border:none;transition:.15s;
...
border-radius:10px;padding:10px;margin-bottom:8px;text-decoration:none;color:inherit;}
...
.chat-header .pn{font-weight:800;font-size:15px}.chat-header .pu{color:var(--muted);font-size:12px}
...
.bubble{max-width:75%;padding:10px 14px;border-radius:14px;font-size:15px;line-height:1.5;word-wrap:break-word;}
...
.bubble-tools{display:flex;gap:4px;opacity:.35;transition:.15s;margin-top:-2px}
.bubble-wrap:hover .bubble-tools{opacity:1}
.mini-btn{background:rgba(255,255,255,.9);border:1px solid var(--border);color:var(--text);
width:26px;height:26px;border-radius:50%;font-size:13px;cursor:pointer;padding:0;margin:0;
display:inline-flex;align-items:center;justify-content:center;line-height:1;box-shadow:0 1px 3px rgba(0,0,0,.08)}
.mini-btn:hover{background:#eef2ff;border-color:#c7d2fe}
...
border-radius:999px;padding:2px 8px;font-size:14px;box-shadow:0 1px 4px rgba(0,0,0,.1);line-height:1.2;
z-index:2}
...
.emoji-picker #emoji-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(36px,1fr));gap:4px}
...
.role-badge{display:inline-block;padding:2px 10px;border-radius:999px;font-size:11px;font-weight:800;margin-right:6px}
...
.group-id-box .gid-value{font-family:'Courier New',monospace;font-size:22px;font-weight:900;color:#1e3a8a;
letter-spacing:3px}
...
def render_page(body, title=None, **ctx):
    ...
    return render_template_string(
        ...
        "if(active&&active.tagName==='INPUT'&&active.id==='chat-body-input')return;"
...
def friends_list():
    ...
    for f in friends:
        ...
        <a href="{url_for('view_profile', username=f.username)}" style="flex:1;text-decoration:none;color:inherit">
          <div class="li-name">{f.short_name}</div><div class="li-sub">@{f.username}</div></a>
...
def blocked_list():
    ...
    for b in rows:
        ...
        <a href="{url_for('view_profile', username=u.username)}" style="flex:1;text-decoration:none;color:inherit">
          ...
          <div class="li-sub">@{u.username} · حظر في {fmt_sd(b.created_at, '%Y-%m-%d')}</div></a>
        <div class="li-actions">
...
def archived_list():
    ...
    for a in rows:
        ...
        <a href="{url_for('chat_with', username=u.username)}" style="flex:1;text-decoration:none;color:inherit">
          <div class="li-name">📦 {u.short_name}</div>
          <div class="li-sub">@{u.username}</div></a>
        <div class="li-actions">
...
def delete_account():
    ...
    <p style="font-size:13px;color:#7f1d1d;line-height:1.7">
      سيتم حذف <b>كل شيء</b> نهائيًا: حسابك، حالاتك، رسائلك، صداقاتك، مجموعاتك، وكل بياناتك.</p></div>
...
def discover():
    ...
    for u in pagination.items:
        ...
        rows += f"""<div class="list-item" style="cursor:default">
          <a href="{url_for('view_profile', username=u.username)}" style="display:flex;align-items:center;gap:10px;flex:1;text-decoration:none;color:inherit">
          ...
          <div class="li-actions">
            <button class="copy-btn" style="padding:4px 8px;font-size:11px" onclick="copyText('{u.public_id}', this)">ID</button>
            {action}</div></div>"""
    if not rows:
...
def post_status():
    if request.method == "POST":
        ...
        db.session.add(Status(user_id=current_user.id, text=text[:300], caption=caption[:300],
                              media=media_fn, media_type=media_type, privacy="public",
    ...
    <div class="privacy-note" style="background:#dbeafe;border-color:#93c5fd;color:#1e40af">
      🌍 جميع الحالات عامة ويراها كل المستخدمين.</div>
...
def status_viewers(sid):
    ...
    for v in views:
        ...
        <a href="{url_for('view_profile', username=u.username)}" style="flex:1;text-decoration:none;color:inherit">
          <div class="li-name">{u.short_name}</div>
          <div class="li-sub">@{u.username} · {fmt_sd(v.viewed_at, '%H:%M')}</div></a></div>"""
    if not rows:
...
# ═══════════════════ الدردشة الفردية (v6.3 مع الرد والإيموجي) ═══════════════════
@app.route("/chats")
@login_required
def chats():
    badge = badge_html()
    ...
    for f in friends:
        ...
        rows += f"""<div class="list-item">
          <img class="avatar-sm" src="{avatar_url(f)}" onclick="openAvatar('{avatar_url(f)}')">
          <a href="{url_for('chat_with', username=f.username)}" style="flex:1;text-decoration:none;color:inherit">
...
def render_bubble_chat(m, current_user, peer):
    ...
    if m.reply_to_id:
        ...
        if orig:
            orig_text = orig.body or ("📷 صورة" if orig.media_type == "image" else "🎥 فيديو" if orig.media_type == "video" else "مرفق")
    ...
    return f"""<div class="bubble-wrap">
      <div class="bubble {cls}" id="msg-{m.id}">
        {reply_html}{media_html}{body_html}
        <span class="t">{fmt_sd(m.created_at, "%H:%M")}</span>
        {reaction_html}
      </div>
      <div class="bubble-tools">
        <button type="button" class="mini-btn" onclick="setReply({m.id}, '{body_esc_js}', '{html_escape_text("أنت" if m.sender_id == current_user.id else peer.short_name)}')" title="رد">↩</button>
        <button type="button" class="mini-btn" onclick="openEmojiPicker({m.id})" title="تفاعل">😊</button>
      </div>
    </div>"""
...
def chat_with(username):
    ...
    <div class="card">
      ...
      <form class="chat-input" method="POST" enctype="multipart/form-data" id="chat-form">
        ...
        <button type="button" class="icon-btn" onclick="document.getElementById('chat-media-input').click()">📎</button>
        <button type="button" class="icon-btn" onclick="toggleEmojiQuick()">😊</button>
    ...
    {emoji_script(url_for('chat_react', username=peer.username))}
...
function setReply(mid, body, name){{
  ...
  document.getElementById('chat-body-input').focus();
}}
...
def render_group_bubble(m, current_user):
    ...
    if m.media:
        ...
        if m.media_type == "video":
            ...
            media_html = f'<img src="{url}" style="max-width:100%;border-radius:10px;margin-bottom:6px;max-height:320px;cursor:pointer" onclick="openAvatar(\'{url}\')">'
    body_html = f'<div>{m.body}</div>' if m.body else ""
    sender_name = m.sender.short_name if m.sender else "?"
    sender_html = ""
    if m.sender_id != current_user.id:
        sender_html = f'<div style="font-size:11px;font-weight:700;color:#2563eb;margin-bottom:4px">{html_escape_text(sender_name)}</div>'
    reply_html = ""
    if m.reply_to_id:
    ...
    body_esc_js = (m.body or "").replace("\\", "\\\\").replace("'", "\\'").replace("\n", " ")[:60]
    return f"""<div class="bubble-wrap">
      <div class="bubble {cls}" id="gmsg-{m.id}">
        {reply_html}{sender_html}{media_html}{body_html}
        <span class="t">{fmt_sd(m.created_at, "%H:%M")}</span>
        {reaction_html}
      </div>
      <div class="bubble-tools">
        <button type="button" class="mini-btn" onclick="setReplyG({m.id}, '{body_esc_js}', '{html_escape_text("أنت" if m.sender_id == current_user.id else sender_name)}')" title="رد">↩</button>
        <button type="button" class="mini-btn" onclick="openEmojiPickerG({m.id})" title="تفاعل">😊</button>
      </div>
    </div>"""
...
function setReplyG(mid, body, name){{
  ...
  document.getElementById('rp-body').textContent=body||'📎 مرفق';
  document.getElementById('reply-preview').style.display='flex';
  document.getElementById('group-body-input').focus();
}}
function cancelReply(){{
  document.getElementById('reply_to_id').value='';
  document.getElementById('reply-preview').style.display='none';
...
def group_avatar_full(gid):
    ...
    is_admin = is_group_admin_or_owner(gid, current_user.id)
    ...
    <div class="card center">
      ...
      <div class="username">ID: {g.public_id}</div>
      <div class="actions" style="margin-top:14px">
        {edit_btn}
        <a class="btn" href="{url_for('group_view', gid=gid)}">← رجوع للمجموعة</a>
      </div>
...
def group_view(gid):
    ...
    for m in members:
        ...
        members_html += f"""<div class="list-item" style="cursor:default">
          ...
          <a href="{url_for('view_profile', username=u.username)}" style="flex:1;text-decoration:none;color:inherit">
            ...
            <div class="li-sub">@{u.username}</div></a>
          <div class="li-actions">{actions}</div></div>"""
    admin_controls = ""
    if is_admin:
    ...
    <div class="card center">
      {avatar_html}
      {('<div style="font-size:11px;color:#9ca3af;margin-top:6px">🔍 اضغط على الصورة لعرضها كاملة' + (' — يمكنك تغييرها من زر تعديل المجموعة' if is_admin else '') + '</div>') if g.avatar else ''}
...
def group_requests(gid):
    ...
    for r in reqs:
        ...
        rows += f"""<div class="list-item" style="cursor:default">
          ...
          <a href="{url_for('view_profile', username=u.username)}" style="flex:1;text-decoration:none;color:inherit">
            <div class="li-name">{u.short_name}</div>
            <div class="li-sub">@{u.username} · {fmt_sd(r.created_at)}</div></a>
          <div class="li-actions">
...
def group_accept_request(gid, rid):
    ...
    if not me or me.role not in ("owner", "admin"):
        abort(403)
    r = GroupJoinRequest.query.get_or_404(rid)
    if r.group_id != gid:
        abort(404)
    uid = r.user_id
    ...
    <div class="card">
      <div class="chat-header">
        ...
        <div><div class="pn">{g.name}</div>
          <div class="pu">ID: {g.public_id}
            <button class="copy-btn" style="padding:2px 6px;font-size:10px" onclick="copyText('{g.public_id}', this)">نسخ</button>
          </div></div>
      ...
      <form class="chat-input" method="POST" enctype="multipart/form-data">
        ...
        <input type="file" name="media" id="chat-media-input" accept="image/*,video/*"
               style="display:none" onchange="previewChatMedia(this)">
        <button type="button" class="icon-btn" onclick="document.getElementById('chat-media-input').click()">📎</button>
        <button type="button" class="icon-btn" onclick="toggleEmojiQuickG()">😊</button>
...
def support_user_detail(uid):
    ...
    status_badge = '<span class="blocked-badge">🚫 محظور</span>' if u.is_banned else '<span style="color:#16a34a;font-weight:700">✅ نشط</span>'

    reports_against_html = ""
    if reports_against:
        for r in reports_against:
            rep = db.session.get(User, r.reporter_id) if r.reporter_id else None
            reporter_txt = f'@{rep.username} ({rep.short_name})' if rep else 'مجهول'
            reporter_pub = rep.public_id if rep else '—'
            reports_against_html += f"""<div class="msg">
              <div class="meta">🪪 صاحب الشوكة: <b>{reporter_txt}</b> · ID: {reporter_pub} · {fmt_sd(r.created_at)}</div>
    ...
    report_target_btn = (f'<a class="btn" href="{url_for("report_user", username=u.username)}">'
                         f'🚨 إبلاغ عن هذا المستخدم (كطرف آخر)</a>')
    ...
    <div class="card center">
      ...
      <div class="uid">ID: {u.public_id}</div>
      ...
      <div class="acct-box" style="margin-top:12px">
        ...
        <div class="acct-row"><span class="k">REPORTS AGAINST</span><span class="v" style="color:#dc2626;font-weight:800">{u.reports_count or 0}</span></div>
        <div class="acct-row"><span class="k">PROFILE VIEWS</span><span class="v">{u.profile_views or 0}</span></div>
      </div>
      <div class="actions">
        <a class="btn" href="{url_for('view_profile', username=u.username)}">👤 فتح البروفايل العام</a>
        {report_target_btn}
        {ban_btn}
        <a class="btn" href="{url_for('support_users')}">← رجوع لإدارة المستخدمين</a>
      </div>
    </div>
    <div class="card"><h2>📊 نشاط الحساب</h2>
    <div class="acct-box" style="direction:rtl;text-align:right">
      <div class="acct-row"><span class="k">الحالات المنشورة</span><span class="v" style="font-family:inherit">{statuses_count}</span></div>
      <div class="acct-row"><span class="k">رسائل دردشة أرسلها</span><span class="v" style="font-family:inherit">{chat_sent}</span></div>
      <div class="acct-row"><span class="k">رسائل دردشة استقبلها</span><span class="v" style="font-family:inherit">{chat_received}</span></div>
      ...
      <div class="acct-row"><span class="k">مجموعات عضو بها</span><span class="v" style="font-family:inherit">{groups_member}</span></div>
      <div class="acct-row"><span class="k">الرسائل المجهولة الواردة</span><span class="v" style="font-family:inherit">{msgs_inbox}</span></div>
      <div class="acct-row"><span class="k">الأجهزة المتصلة</span><span class="v" style="font-family:inherit">{devices_count}</span></div>
    </div></div>
    <div class="card"><h2>🚨 البلاغات الواردة ضده ({len(reports_against)})</h2>
    <div style="font-size:12px;color:#6b7280;margin-bottom:10px">
      ⚠️ زر الحظر هنا يُطبَّق على <b>صاحب الشوكة</b> (المُبلِّغ) — استخدمه فقط إذا ثبت أن البلاغ كاذب.
    </div>
    {reports_against_html}</div>
    <div class="card"><h2>📤 البلاغات التي أرسلها ({len(reports_by_me)})</h2>
    {reports_by_html}</div>
...
def support_reports():
        ...
        if user_reports:
            ...
            for r in user_reports:
                ...
                target_ban_btn = (f'<a class="btn btn-sm btn-primary" href="{url_for("support_unban_user", uid=target.id)}">✅ إلغاء حظر الهدف</a>'
                                  ...
                                  f'<a class="btn btn-sm btn-danger" href="{url_for("support_ban_user", uid=target.id)}" onclick="return confirm(\'حظر هذا المستخدم (الهدف) نهائياً؟\')">🚫 حظر الهدف</a>')
                reporter_box = ""
                if reporter:
                    reporter_status = '<span class="blocked-badge">محظور</span>' if reporter.is_banned else ""
                    if reporter.is_banned:
                        reporter_ban_btn = f'<a class="btn btn-sm btn-primary" href="{url_for("support_unban_user", uid=reporter.id)}">✅ إلغاء حظر المُبلِّغ</a>'
                    else:
                        reporter_ban_btn = (f'<a class="btn btn-sm btn-danger" '
                                            f'href="{url_for("support_ban_user", uid=reporter.id)}" '
                                            f'onclick="return confirm(\'⚠️ هل أنت متأكد؟ هذا سيعاقب صاحب الشوكة (المُبلِّغ) نفسه. استخدمه فقط إذا ثبت أن البلاغ كاذب.\')">'
                                            f'🚫 حظر صاحب الشوكة</a>')
                    reporter_box = f"""<div style="background:#fef2f2;border:1px dashed #fecaca;
                        border-radius:8px;padding:10px;margin-top:8px">
                        <div style="font-size:12px;font-weight:800;color:#991b1b;margin-bottom:6px">
                          🪪 صاحب الشوكة (المُبلِّغ)</div>
                        <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
                          <img class="avatar-sm" src="{avatar_url(reporter)}" onclick="openAvatar('{avatar_url(reporter)}')">
                          <a href="{url_for('support_user_detail', uid=reporter.id)}" style="flex:1;text-decoration:none;color:inherit">
                            <div style="font-weight:700">@{reporter.username} <span style="color:#6b7280;font-weight:400">({reporter.short_name})</span> {reporter_status}</div>
                            <div style="font-size:11px;color:#6b7280">ID: {reporter.public_id} · بلاغات أرسلها: {Report.query.filter_by(reporter_id=reporter.id).count()}</div>
                          </a>
                          <a class="btn btn-sm" href="{url_for('support_user_detail', uid=reporter.id)}">👁 فحص</a>
                          {reporter_ban_btn}
                        </div>
                      </div>"""
                users_html += f"""<div class="list-item" style="cursor:default;flex-wrap:wrap;border-right:4px solid #2563eb">
                  <img class="avatar-sm" src="{avatar_url(target)}" onclick="openAvatar('{avatar_url(target)}')">
                  <div style="flex:1;min-width:200px">
                    <div class="li-name">🎯 @{target.username} <span style="color:#6b7280;font-weight:400">({target.short_name})</span> {reports_badge} {target_status}</div>
                    ...
                    <div style="font-size:13px;margin-top:6px;color:#374151;background:#fef3c7;border-radius:6px;padding:6px 10px">
                      💬 {html_escape_text(r.reason) or 'بدون سبب مذكور'}</div>
                    {reporter_box}
                  </div>
                  <div class="li-actions" style="flex-direction:column;gap:4px">
                    <a class="btn btn-sm" href="{url_for('support_user_detail', uid=target.id)}">👁 فحص الحساب</a>
                    <a class="btn btn-sm" href="{url_for('view_profile', username=target.username)}">👤 عرض الهدف</a>
                    {target_ban_btn}
            ...
            for r in group_reports:
                ...
                if reporter:
                    ...
                    if reporter.is_banned:
                        ...
                        reporter_ban_btn = (f'<a class="btn btn-sm btn-danger" '
                                            f'href="{url_for("support_ban_user", uid=reporter.id)}" '
                                            f'onclick="return confirm(\'⚠️ هل أنت متأكد؟ هذا سيعاقب صاحب الشوكة (المُبلِّغ) نفسه.\')">'
                                            f'🚫 حظر صاحب الشوكة</a>')
                    reporter_box = f"""<div style="background:#fef2f2;border:1px dashed #fecaca;
                        border-radius:8px;padding:10px;margin-top:8px">
                        <div style="font-size:12px;font-weight:800;color:#991b1b;margin-bottom:6px">🪪 صاحب الشوكة (المُبلِّغ)</div>
                        <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
                          <img class="avatar-sm" src="{avatar_url(reporter)}" onclick="openAvatar('{avatar_url(reporter)}')">
                          <a href="{url_for('support_user_detail', uid=reporter.id)}" style="flex:1;text-decoration:none;color:inherit">
                            <div style="font-weight:700">@{reporter.username} <span style="color:#6b7280;font-weight:400">({reporter.short_name})</span> {reporter_status}</div>
                            <div style="font-size:11px;color:#6b7280">ID: {reporter.public_id}</div></a>
                          <a class="btn btn-sm" href="{url_for('support_user_detail', uid=reporter.id)}">👁 فحص</a>
                          {reporter_ban_btn}
                        </div></div>"""
                groups_html += f"""<div class="list-item" style="cursor:default;flex-wrap:wrap;border-right:4px solid #f59e0b">
                  <img class="avatar-sm" src="{group_avatar_url(g)}" style="border-radius:12px">
                  <div style="flex:1;min-width:200px">
                    <div class="li-name">◉ {g.name} <span style="color:#6b7280;font-weight:400">ID: {g.public_id}</span> {group_status}</div>
                    <div class="li-sub">المُبلِّغ: {('@' + reporter.username) if reporter else 'مجهول'} · {fmt_sd(r.created_at)} · {GroupMember.query.filter_by(group_id=g.id).count()} عضو</div>
                    <div style="font-size:13px;margin-top:6px;color:#374151;background:#fef3c7;border-radius:6px;padding:6px 10px">
                      💬 {html_escape_text(r.reason) or 'بدون سبب مذكور'}</div>
                    {reporter_box}
                  </div>
    ...
    <a class="link-center" href="{url_for('support_dashboard')}">← رجوع للوحة</a></div>""", title="البلاغات")
...
def support_users():
    ...
    for u in users:
        ...
        rows += f"""<div class="list-item" style="cursor:default">
          ...
          <div style="flex:1"><div class="li-name">{u.short_name} {status}</div>
          <div class="li-sub">@{u.username} · ID: {u.public_id} · بلاغات: {u.reports_count or 0}</div></div>
          <div class="li-actions">
            <a class="btn btn-sm" href="{url_for('support_user_detail', uid=u.id)}">👁 فحص</a>
            {action}
          </div></div>"""
    if not rows:
        rows = '<p class="empty">لا نتائج.</p>'
...
def support_groups():
    ...
    for g in groups:
        ...
        rows += f"""<div class="list-item" style="cursor:default">
          ...
          <div class="li-sub">ID: {g.public_id} · {GroupMember.query.filter_by(group_id=g.id).count()} عضو</div></div>
          <div class="li-actions">
            <a class="btn btn-sm" href="{url_for('group_view', gid=g.id)}">عرض</a>
            {action}
          </div></div>"""
