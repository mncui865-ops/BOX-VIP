# -*- coding: utf-8 -*-
"""
FBI SUDANESE — v6.0 (Railway Ready)
- صفحة حظر مخصصة + زر خروج
- حذف الأرقام "2" من أزرار التواصل
- بلاغات تعرض صاحب الشوكة + إمكانية حظره
- مجموعات كاملة (تعديل/حذف/اسم/وصف/صورة)
"""
...
.social-btn.tg{background:#229ED9;color:#fff}.social-btn.tg2{background:#0088cc;color:#fff}
.social-btn.fb{background:#1877F2;color:#fff}.social-btn.fb2{background:#0d65d9;color:#fff}
...
def social_box_html():
  ...
  <div class="social-btns">
    ...
    <a class="social-btn tg2" href="{TELEGRAM_URL}" target="_blank" rel="noopener">
      <svg viewBox="0 0 24 24" fill="currentColor"><path d="M9.78 18.65l.28-4.23 7.68-6.92c.34-.31-.07-.46-.52-.19L7.74 13.3 3.64 12c-.88-.25-.89-.86.2-1.3l15.97-6.16c.73-.33 1.43.18 1.15 1.3l-2.72 12.81c-.19.91-.74 1.13-1.5.71L12.6 16.3l-1.99 1.93c-.23.23-.42.42-.83.42z"/></svg>
      Telegram
    </a>
    ...
    <a class="social-btn fb2" href="{FACEBOOK_URL}" target="_blank" rel="noopener">
      <svg viewBox="0 0 24 24" fill="currentColor"><path d="M22 12.06C22 6.5 17.52 2 12 2S2 6.5 2 12.06c0 5.02 3.66 9.18 8.44 9.94v-7.03H7.9v-2.91h2.54V9.85c0-2.51 1.49-3.9 3.77-3.9 1.09 0 2.24.2 2.24.2v2.47h-1.26c-1.24 0-1.63.78-1.63 1.57v1.87h2.78l-.44 2.91h-2.34V22c4.78-.76 8.44-4.92 8.44-9.94z"/></svg>
      Facebook
    </a>
  </div>
...
def groups_list():
    ...
    <form method="GET" style="display:flex;gap:8px;margin-bottom:12px">
      <input name="q" value="{q}" placeholder="اسم المجموعة أو المعرف" style="flex:1;margin:0">
      <button type="submit" style="width:auto;padding:12px 20px;margin:0">بحث</button></form>
...
def group_create():
    if request.method == "POST":
        ...
        while Group.query.filter_by(public_id=pid).first():
            pid = gen_public_id()
        g = Group(name=name, description=description, public_id=pid, owner_id=current_user.id)
        db.session.add(g)
        db.session.flush()
        file = request.files.get("avatar")
        if file and file.filename:
...
def group_view(gid):
    ...
    <div class="card center">
      ...
      <div style="color:#6b7280;font-size:13px;margin-top:8px">
        {len(members)} عضو · أنشئت {fmt_sd(g.created_at, '%Y-%m-%d')}</div>
...
def group_chat(gid):
    ...
    <div class="card">
      <div class="chat-header">
        ...
        <div style="margin-right:auto;display:flex;gap:6px">{admin_link}</div>
      </div>
      {add_form}
...
def support_groups():
    ...
    for g in groups:
        ...
        rows += f"""<div class="list-item" style="cursor:default">
          ...
          <div class="li-sub">@{g.public_id} · {GroupMember.query.filter_by(group_id=g.id).count()} عضو</div></div>
          <div class="li-actions">
            <a class="btn btn-sm" href="{url_for('group_view', gid=g.id)}">عرض</a>
            {action}
          </div></div>"""
