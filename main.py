...
"""
╔══════════════════════════════════════════════════════════╗
║              𝙵𝙱𝙸 𝚂𝚄𝙳𝙰𝙽𝙴𝚂𝙴  —  v3.5 (Railway Ready)      ║
╠══════════════════════════════════════════════════════════╣
║  ✅ حالات + دردشة + رسائل مجهولة + أصدقاء                ║
║  ✅ بحث باليوزر أو بالـ ID                                ║
║  ✅ حالات متعددة + تنقل بينها                             ║
║  ✅ صفحة "اكتشف الأشخاص"                                  ║
║  ✅ حذف صورة البروفايل + نسخ اليوزر والـ ID              ║
║  ✅ دعم Railway Volumes + PostgreSQL                      ║
║  ✅ حظر / إلغاء حظر + حذف صديق + أرشفة + مسح حساب        ║
║  ✅ إرسال صور وفيديوهات في الدردشة                        ║
║  ✅ نظام بلاغات تلقائي (200 بلاغ = حظر فوري)              ║
║  ✅ تحرير اليوزر تلقائيًا بعد 24 ساعة                     ║
║  ✅ المحظور يستطيع إنشاء حساب جديد عادي                  ║
║  🆕 خصوصية الحالة: عامة / أصدقاء فقط                     ║
║  🆕 خيار إظهار مشاهدين الحالة لصاحبها                    ║
║  🆕 أي مستخدم مسجّل يرى البروفايل ويضيف صديق             ║
╚══════════════════════════════════════════════════════════╝
"""
...
if VOLUME_MOUNT and os.path.isdir(VOLUME_MOUNT):
    ...
    for sub in ["uploads", "uploads/avatars", "uploads/statuses", "uploads/chats"]:
...
for d in (AVATAR_DIR, STATUS_DIR, CHAT_DIR):
...
class Status(db.Model):
    ...
    # 🆕 خصوصية الحالة: public / friends
    privacy = db.Column(db.String(10), default="public")
    # 🆕 هل يظهر المشاهدون لصاحب الحالة؟
...
def process_auto_ban(user):
    """حظر تلقائي كامل: إغلاق الحساب + مسح كل المحتوى + تحرير اليوزر بعد 24 ساعة"""
    ...
    # مسح الحالات وملفاتها ومشاهداتها
    ...
    db.session.commit()
    print(f"[AUTO-BAN] User #{user.id} banned. Username '{user.original_username}' "
          f"will be released at {user.username_release_at}")
    return True


def release_pending_usernames():
    """تحرير اليوزرات المحجوزة بعد مرور 24 ساعة"""
    now = now_utc_naive()
    ...
    if pending:
        db.session.commit()
        print(f"[USERNAME-RELEASE] Released {len(pending)} usernames")
    ...
    with app.app_context():
        ...
        try:
            ...
            for col, typ in [
                ...
                ("show_viewers","BOOLEAN DEFAULT FALSE"),
            ]:
            ...
            if "chat_message" in insp.get_table_names():
                chat_cols = [c["name"] for c in insp.get_columns("chat_message")]
                for col, typ in [
                    ("media","VARCHAR(300) DEFAULT ''"),
                    ("media_type","VARCHAR(10) DEFAULT ''"),
                ]:
                    if col not in chat_cols:
                        db.session.execute(text(f"ALTER TABLE chat_message ADD COLUMN {col} {typ}"))
                        db.session.commit()

            for idx in [
                'CREATE INDEX IF NOT EXISTS idx_user_recovery ON "user"(recovery_hash)',
                'CREATE INDEX IF NOT EXISTS idx_user_reports ON "user"(reports_count)',
                'CREATE INDEX IF NOT EXISTS idx_user_release ON "user"(username_release_at)',
                'CREATE INDEX IF NOT EXISTS idx_status_privacy ON status(privacy)',
                'CREATE INDEX IF NOT EXISTS idx_statusview_status ON status_view(status_id)',
            ]:
                try:
...
.btn-primary::after{content:"";position:absolute;inset:0;border-radius:10px;
  box-shadow:0 0 0 0 rgba(37,99,235,.4);transition:.2s;pointer-events:none}
.btn-primary:hover::after{box-shadow:0 0 0 6px rgba(37,99,235,.12)}
...
.status-item .vid-tag{position:absolute;bottom:0;right:0;background:rgba(0,0,0,.7);
  ...
  font-weight:700;}
.status-item .lock-tag{position:absolute;bottom:0;left:0;background:rgba(37,99,235,.85);
  color:#fff;font-size:9px;padding:1px 5px;border-radius:8px;border:2px solid #fff;
  font-weight:700;}
...
.status-owner-actions .sbtn.viewers:hover{background:rgba(22,163,74,1);text-decoration:none}
...
.archived-badge{background:#f3f4f6;color:#6b7280;font-size:11px;
  padding:2px 8px;border-radius:999px;font-weight:700;border:1px solid #e5e7eb}
.danger-zone{border:2px dashed #fecaca;border-radius:12px;padding:16px;
  margin-top:14px;background:#fef2f2;}
.danger-zone h3{color:#991b1b;font-size:14px;font-weight:800;margin-bottom:8px;}

/* 🆕 خيارات الخصوصية للحالة */
.privacy-choose{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:8px}
.privacy-opt{display:flex;flex-direction:column;align-items:center;gap:4px;
  padding:14px 10px;border:2px solid var(--border);border-radius:12px;
  cursor:pointer;transition:.15s;background:var(--surface);text-align:center;
  position:relative}
.privacy-opt:hover{border-color:#93c5fd;background:#f0f9ff}
.privacy-opt input{position:absolute;opacity:0;pointer-events:none}
.privacy-opt input:checked ~ .po-ic{transform:scale(1.1)}
.privacy-opt:has(input:checked){border-color:var(--blue);
  background:#eff6ff;box-shadow:0 0 0 3px rgba(37,99,235,.12)}
.privacy-opt .po-ic{font-size:26px;line-height:1}
.privacy-opt .po-t{font-weight:800;font-size:14px;color:var(--text)}
.privacy-opt .po-d{font-size:11px;color:var(--muted)}

/* 🆕 switch لإظهار المشاهدين */
...
.switch-row:hover{background:#f3f4f6}
</style>
"""
...
def unfriend_user(username):
    target = User.query.filter_by(username=username.lower().lstrip("@")).first()
    if not target:
        flash("المستخدم غير موجود.","error"); return redirect(url_for("friends_list"))
...
def block_user(username):
    ...
    ArchivedChat.query.filter_by(user_id=current_user.id, peer_id=target.id)\
        .delete(synchronize_session=False)
    db.session.commit()
...
def blocked_list():
    ...
    <div class="card">
      ...
      <p style="color:#6b7280;font-size:12px;text-align:center;margin-bottom:10px">
        المحظورون لا يمكنهم مراسلتك أو رؤية بروفايلك الكامل.</p>
      {html}
      <a class="link-center" href="{url_for('profile_me')}">رجوع</a>
    </div>""", title="المحظورون")
...
def archived_list():
    ...
    for a in rows:
        ...
        <a href="{url_for('chat_with', username=u.username)}"
           style="flex:1;text-decoration:none;color:inherit">
          <div class="li-name">📦 {u.short_name}</div>
          <div class="li-sub">@{u.username} · أُرشف في {fmt_sd(a.created_at, '%Y-%m-%d')}</div>
        </a>
        <div class="li-actions">
          <a class="btn btn-sm btn-primary"
             href="{url_for('chat_with', username=u.username)}">فتح</a>
          <a class="btn btn-sm btn-danger"
             href="{url_for('unarchive_chat', username=u.username)}"
             onclick="return confirm('إلغاء الأرشفة؟')">↩</a>
        </div></div>"""
    if not html:
        html = '<p class="empty">الأرشيف فارغ.</p>'
    return render_page(FLASH_BLOCK + topbar_html(badge) + f"""
    <div class="card">
      <h2>📦 المحادثات المؤرشفة ({len(rows)})</h2>
      <p style="color:#6b7280;font-size:12px;text-align:center;margin-bottom:10px">
        المحادثات المؤرشفة مخفية من قائمة الدردشات الرئيسية.</p>
      {html}
      <a class="link-center" href="{url_for('chats')}">← رجوع للدردشات</a>
    </div>""", title="الأرشيف")
...
def delete_account():
    if request.method == "POST":
        ...
        if not check_password_hash(current_user.password_hash, password):
            flash("كلمة السر خاطئة. لا يمكن إتمام الحذف.","error")
        ...
        resp = make_response(redirect(url_for("login")))
        resp.delete_cookie("device_token")
        flash("تم مسح حسابك بالكامل. نأمل رؤيتك مجددًا.","success")
        return resp
    ...
    <div class="card">
      ...
      <div class="danger-zone">
        ...
        <p style="font-size:13px;color:#7f1d1d;line-height:1.7">
          سيتم حذف <b>كل شيء</b> نهائيًا ولا يمكن التراجع:<br>
          • حسابك وبياناتك<br>
          • جميع حالاتك وصورك<br>
          • جميع رسائلك ودردشاتك<br>
          • جميع صداقاتك<br>
          • سجل الحظر والأرشفة<br>
          • الأجهزة المتصلة
        </p>
      </div>
...
def post_status():
    ...
    <form method="POST" enctype="multipart/form-data">
      ...
      <label>🔒 من يمكنه رؤية الحالة؟</label>
      <div class="privacy-choose">
        <label class="privacy-opt">
          <input type="radio" name="privacy" value="public" checked>
          <span class="po-ic">🌍</span>
          <span class="po-t">عامة</span>
          <span class="po-d">الكل يشاهدها</span>
        </label>
        <label class="privacy-opt">
          <input type="radio" name="privacy" value="friends">
          <span class="po-ic">👥</span>
          <span class="po-t">أصدقاء فقط</span>
          <span class="po-d">أصدقاؤك فقط</span>
        </label>
      </div>
      ...
      <p style="font-size:12px;color:#6b7280;margin-top:6px">
        عند التفعيل، يمكنك أنت فقط رؤية عدد وقائمة من شاهد حالتك.
      </p>
...
def edit_status(sid):
    ...
    if request.method == "POST":
        ...
        return redirect(url_for("feed"))
    checked_pub = "checked" if s.privacy == "public" else ""
    checked_fr = "checked" if s.privacy == "friends" else ""
    ...
    <form method="POST">
      ...
      <div class="privacy-choose">
        <label class="privacy-opt">
          <input type="radio" name="privacy" value="public" {checked_pub}>
          <span class="po-ic">🌍</span>
          <span class="po-t">عامة</span>
          <span class="po-d">الكل يشاهدها</span>
        </label>
        <label class="privacy-opt">
          <input type="radio" name="privacy" value="friends" {checked_fr}>
          <span class="po-ic">👥</span>
          <span class="po-t">أصدقاء فقط</span>
          <span class="po-d">أصدقاؤك فقط</span>
        </label>
...
def status_viewers(sid):
    ...
    if not s.show_viewers:
        flash("لم تفعّل خيار عرض المشاهدين لهذه الحالة.", "error")
        return redirect(url_for("feed"))
    ...
    <div class="card">
      ...
      <div class="privacy-note" style="background:#dbeafe;border-color:#93c5fd;color:#1e40af">
        🔒 هذه القائمة تظهر لك فقط لأنك فعّلت خيار إظهار المشاهدين.
      </div>
      {rows}
...
def feed():
    ...
    # فلترة حسب الخصوصية: العامة للجميع + الأصدقاء للأصدقاء
    visible = []
    for s in statuses:
        if s.user_id == current_user.id:
            visible.append(s); continue
        if s.privacy == "friends":
            if s.user_id in friend_ids:
                visible.append(s)
        else:
            visible.append(s)
    statuses = visible
    ...
    # تسجيل المشاهدات لصاحب الحالة فقط لو مفعّل show_viewers
    for s in statuses:
        if s.user_id == current_user.id:
            continue
        if s.show_viewers:
            exists = StatusView.query.filter_by(
                status_id=s.id, viewer_id=current_user.id).first()
            if not exists:
    ...
    for uid, items in by_user.items():
        ...
        lock = '<span class="lock-tag">🔒</span>' if all(i.privacy == "friends" for i in items) else ""
        ...
        status_strip += f"""<a class="status-item" href="javascript:void(0)"
          ...
          <span class="ring">{count_badge}<img src="{av}">{tag}{lock}</span>
    ...
    for uid, items in by_user.items():
        for idx, s in enumerate(items):
            ...
            if s.user_id == current_user.id:
                ...
                if s.show_viewers:
                    ...
                    viewers_btn = (f'<a class="sbtn viewers" '
                                   f'href="{url_for("status_viewers", sid=s.id)}">'
                                   f'👁 {vcount}</a>')
            ...
            priv_label = ('🌍 عامة' if s.privacy == "public" else '👥 أصدقاء فقط')
            status_viewers_html += f"""<div class="status-view" id="sv-{s.id}" style="display:none">
              ...
              <div class="smeta">{s.user.short_name} · {fmt_sd(s.created_at)} · {priv_label}</div>
              {nav_html}
            </div>"""
...
def chats():
    ...
    for f in friends:
        ...
        rows += f"""<div class="list-item">
          ...
          <div style="display:flex;flex-direction:column;align-items:flex-end;gap:4px">
            ...
            <form method="POST" action="{url_for('archive_chat', username=f.username)}"
                  style="margin:0">
              <button type="submit" class="copy-btn" style="padding:2px 8px;font-size:11px"
                      title="أرشفة">📦</button>
            </form>
          </div>
        </div>"""
    ...
    archive_link = ""
    if archived_count:
        archive_link = f'<a class="btn btn-sm" href="{url_for("archived_list")}">📦 الأرشيف ({archived_count})</a>'
...
def chat_with(username):
    ...
    if is_blocked_between(current_user.id, peer.id):
        flash("لا يمكن إتمام العملية (يوجد حظر بينكما).", "error")
        return redirect(url_for("chats"))
    ...
    if request.method == "POST":
        ...
        if file and file.filename:
            ...
            if allowed_video(fn_low) and check_video_magic(file):
                ...
                flash("صيغة الملف غير مدعومة. استخدم صورة أو فيديو.", "error")
    ...
    if is_archived:
        arch_action = f"""<form method="POST"
            action="{url_for('unarchive_chat', username=peer.username)}" style="margin:0">
          <button type="submit" class="btn btn-sm">↩ إلغاء الأرشفة</button></form>"""
    else:
        arch_action = f"""<form method="POST"
            action="{url_for('archive_chat', username=peer.username)}" style="margin:0">
    ...
    <div class="card">
      ...
      <form class="chat-input" method="POST" enctype="multipart/form-data"
            id="chat-form">
        <input type="file" name="media" id="chat-media-input"
               accept="image/*,video/*" style="display:none"
               onchange="previewChatMedia(this)">
        <button type="button" class="icon-btn" title="إرفاق صورة/فيديو"
...
def report_user(username):
    ...
    if request.method == "POST":
        ...
        existing = Report.query.filter_by(
            reporter_id=current_user.id, target_id=target.id).first()
        if existing:
            flash("لقد أبلغت عن هذا المستخدم مسبقًا.", "error")
            return redirect(url_for("view_profile", username=target.username))
        ...
        if target.reports_count >= AUTO_BAN_THRESHOLD and not target.is_banned:
            process_auto_ban(target)
            flash("تم إرسال الإبلاغ. تم حظر الحساب تلقائيًا لتجاوزه حد البلاغات.", "success")
            return redirect(url_for("profile_me"))
    ...
    <div class="card">
      ...
      <div class="privacy-note">
        ⚠️ عند وصول الحساب إلى {AUTO_BAN_THRESHOLD} بلاغًا من مستخدمين مختلفين،
        سيُغلق تلقائيًا ويُمسح كل محتواه، ويتحرر يوزره بعد {USERNAME_RELEASE_HOURS} ساعة.
      </div>
      <form method="POST">
