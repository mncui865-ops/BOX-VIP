...
"""
FBI SUDANESE — v19
- زر إعدادات موحّد (بدون إيموجي)
- توثيق شائك (فيسبوك سيل) + 60 شكل إضافي
- سحابة الحالة المزاجية بجانب الاسم (تختفي وتظهر كل 5 ثواني)
- تحديث لحظي للرسائل والحالات (SSE)
- حذف قسم الهدايا
- صورة القروب تفتح كبيرة عند الضغط
"""
...
# ═══════════════════════════════════════════════════════════════
# GLOW_ICONS (موسّع — يغطي كل ما نحتاج)
# ═══════════════════════════════════════════════════════════════
GLOW_ICONS = {
...
# ═══════════════════════════════════════════════════════════════
# ALL_VERIFY_SHAPES — 360+ شكل SVG (يضاف الشكل الشائك "فيسبوك سيل")
# ═══════════════════════════════════════════════════════════════
...
# ⭐ شكل التوثيق الشائك (فيسبوك سيل — 24 سن)
...
_BASE_SHAPES = [
    ...
    ("facebook_seal", _svg_facebook_seal()),   # ⭐ الشكل الشائك
]
...
# ⭐ 60 شكلاً إضافياً (شائك/نجمي/زاوي) بأشكال هندسية متعددة
for i in range(60):
    angle = (i * 13) % 360
    radius = 5 + (i % 6)
    sides = 4 + (i % 10)
...
# ⭐ 60 شكلاً شوكياً (شبيه بفيسبوك سيل) بزوايا مختلفة
for i in range(60):
    teeth = 16 + (i % 20)
    outer_r = 11
    inner_r = 8 - (i % 3) * 0.5
...
def is_group_admin_or_owner(gid, uid):
    ...
    # ═══════════════════════════════════════════════════════════════
# BASE_STYLE (مع أنماط جديدة للإعدادات والسحابة والشكل الشائك)
# ═══════════════════════════════════════════════════════════════
BASE_STYLE = """
<style>
...
@keyframes moodFloat2{0%,100%{transform:translateY(0);}50%{transform:translateY(-5px);}}
...
def settings_button_html():
    """زر واحد موحّد لجميع الإعدادات (بدون إيموجي)"""
    return f"""
...
# ═══════════════════════════════════════════════════════════════
# PROFILE (الاسم + التوثيق تحت الصورة + السحابة الجانبية)
# ═══════════════════════════════════════════════════════════════
...
def view_profile(username):
    ...
    cu = cover_url(user)
    # ⭐ سحابة الحالة المزاجية بجانب الاسم
    mood_cloud = ""
    if user.mood_text:
    ...
    # ⭐ الاسم + التوثيق تحت الصورة + السحابة بجانبهم
    name_line = f"""
